...
> 阅读对象：测试、实施（主）；生产协同、仓库收货/上架、质量协同人员（顺带）。

## 分组导读（测试 / 实施）

| 你的目的 | 建议阅读 |
| --- | --- |
| 理解制品收货/上架/撤销等如何变成可追溯库存 | **本页**（主文档；以制品收货为主线） |
| 执行制品收货、上架、撤销或查询细节 | [生产管理-维护与查询参考](生产管理-维护与查询参考.md) |
| 售前介绍 | 停在 [WMS 模块首页](../index.md) |

**学习顺序：** 区分本页 vs [发料管理](../06-发料管理/index.md) / [生产收料](../07-生产收料/index.md) → 制品收货主线 → 上架/隔离/拆解返修边界 → [库存管理](../09-库存管理/index.md)。

**配置依赖概览：** DBC 物料/仓位；生产完成事实（非本页维护 MES 计划）；事务类型/计划设置；质量隔离要求。通用对象见[申请、任务与记录模型](../../02-业务模型/01-申请任务记录模型.md)。

## 业务目的与适用范围

WMS 生产管理处理生产结果如何进入、回退或转换仓储库存（制品收货、完工撤销、隔离收货、制品上架，以及拆解、返修等）。**不是** MES 计划/执行模块；发料与生产收料/退料分属对应分组，勿混写。

读完应能说清：本组功能范围、与 MES/发料/收料边界，以及制品入仓验证怎么开。
