...
> 阅读对象：测试、实施（主）；采购协同、仓库、质量协同人员（顺带）。

## 分组导读（测试 / 实施）

| 你的目的 | 建议阅读 |
| --- | --- |
| 弄清退货主线、库存影响与验证场景 | **本页**（主文档） |
| 发起/执行/查询与字段细节 | [采购退货-维护与查询参考](采购退货-维护与查询参考.md) |
| 售前介绍 | 停在 [WMS 模块首页](../index.md) |

**学习顺序：** [采购收货](../03-采购收货/index.md) 结果与异常 → 本页退货闭环 → [库存管理](../09-库存管理/index.md) 核对事务/余额。

**配置依赖概览：** DBC 供应商/物料/仓位；收货来源与质量结论；WMS [系统设置](../02-系统设置/index.md)（事务类型/计划设置，以实测为准）；任务侧数量与扫码门禁见维护参考。

## 业务目的与适用范围

采购退货把不应继续留在企业库存中的来料，按可追溯流程退回供应商（质量不合格、数量/规格不符、超收或业务决定不接收等）。不是直接删除库存，而是保留原因、来源、数量、执行人与库存影响。

读完应能说清：功能范围、申请→任务→记录主线、对库存的影响，以及如何开验证场景。
## 使用前准备
