...
> 阅读对象：测试、实施（主）；生产计划、仓库备料/发料、线边人员（顺带）。

## 分组导读（测试 / 实施）

| 你的目的 | 建议阅读 |
| --- | --- |
| 弄清备料/发料/补料主线与 MES 边界 | **本页**（主文档） |
| 发起/执行/查询与字段细节 | [发料管理-维护与查询参考](发料管理-维护与查询参考.md) |
| 售前介绍 | 停在 [WMS 模块首页](../index.md) |

**学习顺序：** 确认生产需求来源（MES/计划）→ 本页发料执行 → [库存管理](../09-库存管理/index.md)；线边规则勿与 [MES 终端](../../06-MES-生产管理/06-终端操作/index.md) 混用。

**配置依赖概览：** DBC 物料/仓位/包装；生产来源单据；库存可用量与事务类型；任务扫码/数量门禁见维护参考；MES 工单状态以 MES 为准。

## 业务目的与适用范围

发料管理把生产需求转化为仓库可执行的备料、发料和补料，使物料在正确时间、数量与追溯维度进入现场，避免无来源、无记录的随意出库。

读完应能说清：功能范围、用料主线、与 MES 边界，以及如何开验证场景。
## 使用前准备
