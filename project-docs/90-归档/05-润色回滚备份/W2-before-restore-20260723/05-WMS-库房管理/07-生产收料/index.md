...
> 阅读对象：测试、实施（主）；生产现场、仓库收料、质量协同人员（顺带）。

## 分组导读（测试 / 实施）

| 你的目的 | 建议阅读 |
| --- | --- |
| 弄清完工入库主线、退料/隔离分流 | **本页**（主文档） |
| 页面对象边界、列表详情与字段 | [生产收料-维护与查询参考](生产收料-维护与查询参考.md) |
| 售前介绍 | 停在 [WMS 模块首页](../index.md) |

**学习顺序：** 生产完成事实（MES/工单）→ 本页收料/退料/隔离 → [库存管理](../09-库存管理/index.md)；制品收货相邻场景见[生产管理](../08-生产管理/index.md)。

**配置依赖概览：** DBC 物料/仓位；生产来源与质量要求；事务类型与计划设置；合格/隔离等库存状态口径以叶页与实测为准。

## 业务目的与适用范围

生产收料把产线完工制品或半成品转为可追溯的仓库收料结果，连接工单完工、现场交接、质量分流与入账，避免合格品、隔离品、退料等混在同一库存结果中。

读完应能说清：功能范围、产出入仓主线、分流边界，以及如何开验证场景。
## 使用前准备
