...
> 阅读对象：测试、实施（主）；销售协同、仓库备货/发运、客户交付人员（顺带）。

## 分组导读（测试 / 实施）

| 你的目的 | 建议阅读 |
| --- | --- |
| 理解客户交付如何变成可追溯出库与库存结果 | **本页**（主文档：发货主链、关键决策、异常） |
| 发起申请、承接任务、PDA、撤销或追溯细节 | [销售出库-维护与查询参考](销售出库-维护与查询参考.md) |
| 售前介绍 | 停在 [WMS 模块首页](../index.md) |

**学习顺序：** DBC 客户/月台/成品物料 → 本页标准发货主链 → 备货/客户退货/结算出库仅看边界 → [库存管理](../09-库存管理/index.md)。

**配置依赖概览：** DBC 客户、客户月台、物料、仓位；来源销售订单/发货计划（SCP）；事务类型与任务扫码/缺货门禁；通用对象见[申请、任务与记录模型](../../02-业务模型/01-申请任务记录模型.md)。

## 业务目的与适用范围

销售出库把客户交付需求转化为可执行的备货、发运与出库结果，确保「要发什么、发给谁、从哪里发、实际发了多少」可追溯。本页讲标准发货主链；备货、客户退货、结算出库等相邻入口只界定边界，不共用未经证实的状态机。

读完应能说清：功能范围、发货主线、库存影响与异常处理入口。
...
完整选择器、拣选核验与门禁见[维护与查询参考](销售出库-维护与查询参考.md)。相对[采购收货](../03-采购收货/index.md)：对端从供应商变为**客户**；预期从预计入变为**预计出**；执行时强制 **P3 余额核验**。客户/月台/余额选择通例见[通用选择器过滤惯例](../../02-业务模型/12-通用选择器过滤惯例.md)；拣选库位级联见[库位与仓储级联惯例](../../02-业务模型/13-库位与仓储级联惯例.md)。

| 字段/配置点 | 在系统中的作用 | 关键行为要点（取值/范围/联动/门禁） | 维护或操作时要警惕什么 |
| --- | --- | --- | --- |
| 来源销售订单 / 发货计划 | 确定交付范围 | 可选范围受来源状态与业务类型约束（❓） | 选错来源会发错客户或物料 |
| 客户 / 客户月台 | 交付对象与装运点 | 客户常回填锁定；月台与客户一致性有校验线索 | 月台与客户不匹配导致装运错位（`GAP-047`） |
| 出库库存状态范围 | 限制可拣库存 | 未放行/隔离/冻结通常不可发（❓ 完整集合） | “有数发不出”先查状态 |
| 拣选库位 / 余额 | 从哪出库 | 按库存唯一粒度核验可用量，见[库存管理精度与唯一粒度](../../02-业务模型/08-库存管理精度与唯一粒度.md) | 扫错批次/包装 |
| 计划 / 实发数量 | 控制出库量 | 少发/多发受任务配置 | 缺货假装完成 |
| 申请/任务/记录状态 | 门禁 | 处理申请生成任务并建预计出；完成清理预计出 | 备货/退货/结算出库另链，勿混用 |
...
销售出库复用[申请、任务与记录模型](../../02-业务模型/01-申请任务记录模型.md)的通用状态码与动作名；本业务已证实“处理申请 → 任务 + 预计出 → 执行记录 → 事务/余额并清理预计出”。少发、缺货关闭、撤销冲抵的精确状态跳转仍待测试回填，下图对未证实分支保持虚线注释。
...
stateDiagram-v2
    ...
    申请审批中 --> 申请审批驳回: 驳回
    申请审批通过 --> 申请处理中: 处理
    申请处理中 --> 任务待处理: 生成发货任务并建预计出
    任务待处理 --> 任务进行中: 承接
    任务进行中 --> 记录正常: 执行发货<br/>核验余额后出库
    任务待处理 --> 任务关闭: 关闭
    记录正常 --> 记录已撤销: 撤销发货
    记录已撤销 --> 撤销冲抵: 冲抵事务待核验

    note right of 任务进行中
      少发/多发/缺货后的关闭或补发路径待测试确认
      备货、客户退货、结算出库不共用本状态机
    end note
```

库存对象关系见上文「对库存和相关业务的影响」及[库存数据挂接模型](../../02-业务模型/02-库存数据挂接模型.md)。
