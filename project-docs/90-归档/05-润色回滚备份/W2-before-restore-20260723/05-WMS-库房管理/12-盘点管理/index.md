...
> 阅读对象：测试、实施（主）；仓库主管、盘点/库存管理、财务审计协同人员（顺带）。

## 分组导读（测试 / 实施）

| 你的目的 | 建议阅读 |
| --- | --- |
| 理解盘点如何发现并受控处理账实差异 | **本页**（主文档） |
| 建计划、执行盘点、对比差异或调整细节 | [盘点管理-维护与查询参考](盘点管理-维护与查询参考.md) |
| 售前介绍 | 停在 [WMS 模块首页](../index.md) |

**学习顺序：** [库存管理](../09-库存管理/index.md) 账面基准 → 本页计划/任务/实盘/差异/调整 → 勿用[库内作业](../11-库内作业/index.md)替代盘点闭环。

**配置依赖概览：** DBC 仓位与物料、库存状态/批次包装维度；事务类型（调整入账）；盘点执行与盘点调整是不同对象链。通用对象见[申请、任务与记录模型](../../02-业务模型/01-申请任务记录模型.md)。

## 业务目的与适用范围

盘点管理比较系统库存与现场实物，发现差异后按受控流程确认与调整，重点是保留差异来源、过程与调整依据，而不是「把数字改成一样」。

读完应能说清：功能范围、计划→实盘→差异→调整主线，以及与历史误述的边界。
