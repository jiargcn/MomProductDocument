...
> 阅读对象：**测试、实施（主）**；采购结算、供应商财务协同（顺带）。操作见[发票结算-维护与查询参考](发票结算-维护与查询参考.md)。售前停在[模块首页](../index.md)。
...
| 本分组覆盖 | 不在本分组 |
| --- | --- |
| SCP 内发票/索赔/对账日历对象与状态 | 财务总账付款规则；外部回写协议（待确认） |
| 开票数量来源线索 | 仓内收货执行权威（WMS） |
...
| --- | --- |
| 开票与索赔关系 | **本页** |
| 发票/索赔/对账操作 | [发票结算-维护与查询参考](发票结算-维护与查询参考.md) |
| 来源收货数量 | [采购跟踪](../06-采购跟踪/index.md) |
| 开票日历 | [基础数据](../01-基础数据/index.md) 与本组日历页 |
| 售前 | [SCP 模块首页](../index.md)；勿承诺已对接完整财务回写 |
