...
> 阅读对象：测试、实施（主）；计划员、生产主管、MES 实施顾问（顺带）。

## 这一组解决什么问题 / 功能范围

计划管理把外部/计划层的生产需求落实为可下发、可开工的生产工单，并管理计划与执行之间的状态流转。读完应能说清 ERP→生产订单→工单→下发快照主链，以及任务模式/并行组如何改变现场行为。

**范围：** ERP 订单、生产订单、生产工单（计划层）、运行工单、并行组与状态动作。**范围外：** 线边扫码报工见[终端操作](../06-终端操作/index.md)；完工库存落地见 WMS [生产管理](../../05-WMS-库房管理/08-生产管理/index.md) / [生产收料](../../05-WMS-库房管理/07-生产收料/index.md)；路线主数据见[工艺管理](../02-工艺管理/index.md)；WMS「生产计划」菜单与 PS 排程算法不在本组。

## 如何使用本组文档（测试 / 实施）

| 你的目的 | 建议阅读 |
| --- | --- |
| 设计转换 / 拆分 / 下发 / 开工 / 挂起验证 | **本页**（主文档：对象、状态、关键判断） |
| 建单、拆分、下发、开工的操作与选择器 | [计划管理-维护与查询参考](计划管理-维护与查询参考.md) |
| 核对路线版本与运行快照 | [工艺管理](../02-工艺管理/index.md) |
| 查投料发料或完工入库 | WMS 发料 / 生产收料 / 生产管理 |

## 本组学习顺序

1. ERP 转换与生产订单 → 2. 拆分/生成工单并绑路线/资源 → 3. 下发（写运行层+快照）→ 4. 开工/挂起/完工 → 5. 并行组统一动作（若使用）。

## 配置依赖概览

| 依赖 | 影响 | 在哪确认 |
| --- | --- | --- |
| 物料、BOM、可引用工艺路线 | 下发时生成运行快照 | 工艺管理 / DBC |
| 工厂 / 车间 / 产线 / 班次 | 执行资源定位 | DBC 工厂建模 |
| 任务模式（单件 / 批量 / 普通批量） | 追溯粒度与报工入口 | 本页工单 |
| 并行组 | 多工单统一下发/开工 | 本页 |
| ERP 转换 / 锁定状态 | 能否转入或改单 | 本页 ERP 订单 |
