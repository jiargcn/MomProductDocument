...
> 阅读对象：测试、实施（主）；工艺工程师、MES 实施顾问（顺带）。

## 这一组解决什么问题 / 功能范围

工艺管理把「某产品/物料按什么顺序、经哪些工序、受哪些转序与并行约束」维护成可执行的工艺路线，供计划拆分、线边执行与追溯引用。读完应能说清版本/快照边界，以及转序、并行、分派对现场的影响。

**范围：** MES 工艺路线头、图形、工序节点、产品/工位关联、运行快照。**范围外：** DBC 工序字典；检验方案本体（回 QMS）。DBC 侧若仍有工艺建模入口，以[DBC 工艺路线](../../04-DBC-主数据管理/08-工艺建模/02-工艺路线.md)为准：目录位置 ≠ 实现归属。

## 如何使用本组文档（测试 / 实施）

| 你的目的 | 建议阅读 |
| --- | --- |
| 设计路线启停 / 转序 / 分派验证场景 | **本页**（主文档） |
| 新建 / 改版 / 启停路线与选择器细节 | [工艺管理-维护与查询参考](工艺管理-维护与查询参考.md) |
| 看订单如何引用路线与快照 | [计划管理](../03-计划管理/index.md) |
| 查工序主数据 | [DBC 工序管理](../../04-DBC-主数据管理/08-工艺建模/01-工序管理.md) |

## 本组学习顺序

1. 路线头与版本策略 → 2. 图形 / 节点与转序门槛 → 3. 产品与工位配置 → 4. 启用后被计划引用 → 5. 理解运行快照（下发时落库）。

## 配置依赖概览

| 依赖 | 影响 | 在哪确认 |
| --- | --- | --- |
| 物料 / BOM / 版本 | 路线适用对象 | DBC / 物料 |
| 工序与工位主数据 | 节点与可执行工位 | DBC + 本页工位配置 |
| 转序门槛 / 批量转序策略 / 并行上限 | 线边能否流转、多工位并行 | 本页节点配置 |
| 作业分派模式（抢单 / 派单） | 工位如何领作业 | 本页；现场见终端操作 |
| 路线可引用状态 | 计划能否选中 | 本页启停（精确状态集 ❓ `MES-ROUTE`） |
