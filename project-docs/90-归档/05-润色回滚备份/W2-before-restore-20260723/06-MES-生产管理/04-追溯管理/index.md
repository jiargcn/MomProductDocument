...
> 阅读对象：测试、实施（主）；质量、工艺、客诉处理人员（顺带）。

## 这一组解决什么问题 / 功能范围

追溯管理基于报工、工位作业、物料消耗、设备参数与完工事实，提供 **正向（物料→产品）** 与 **逆向（产品/码→过程）** 查询，并维护追溯码规则。读完应能说清查什么入口、无采集则无链路，以及规则如何约束赋码。

**范围外：** 不替代现场报工采集（见[终端操作](../06-终端操作/index.md)）；原始流水核对见[报表统计](../05-报表统计/index.md)。
...
| --- | --- |
| 设计正逆向排查验证场景 | **本页**（主文档） |
| 做客诉/批次排查操作与条件细节 | [追溯管理-维护与查询参考](追溯管理-维护与查询参考.md) |
| 核对报工/完工原始记录 | [报表统计](../05-报表统计/index.md) |

## 本组学习顺序

1. 确认现场已报工/赋码 → 2. 选对正向或逆向起点粒度 → 3. 联查报表记录 → 4. 必要时核对追溯码规则。

## 配置依赖概览

| 依赖 | 影响 | 在哪确认 |
| --- | --- | --- |
| 终端报工 / 消耗 / 赋码事实 | 有无链路可查 | 终端操作 / 报表统计 |
| 追溯码规则（按物料） | 码能否解析 | 本页规则维护 |
| 工单 / 产品 SN / 批次粒度 | 正逆向结果是否可比 | 计划管理任务模式 |
