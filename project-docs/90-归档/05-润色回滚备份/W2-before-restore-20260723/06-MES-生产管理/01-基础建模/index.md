...
> 阅读对象：测试、实施（主）；工艺/线边实施、生产主管（顺带）。

## 这一组解决什么问题 / 功能范围

基础建模维护线边开产前的配置底座：线边客户端、SOP 与挂接、工位技能要求、开工点检模板、线边库/料口映射、生产 BOM（工序用料）。读完应能说清开产前要配什么、配错会拦什么。

**范围外：** 工厂/产线/工位主数据以 [DBC](../../04-DBC-主数据管理/index.md) 为准；人员技能等级主数据当前菜单多在 DBC；设备点检执行主链见 EAM。

## 如何使用本组文档（测试 / 实施）

| 你的目的 | 建议阅读 |
| --- | --- |
| 设计开产前置验证、讲清配置影响 | **本页**（主文档：范围、对象关系、关键判断） |
| 配客户端 / SOP / 点检 / 线边库 / 字段细节 | [基础建模-维护与查询参考](基础建模-维护与查询参考.md) |
| 看路线与工单如何消费这些配置 | [工艺管理](../02-工艺管理/index.md)、[计划管理](../03-计划管理/index.md) |
| 看线边怎么操作 | [终端操作](../06-终端操作/index.md) |

## 本组学习顺序

1. 线边客户端（登录与工位绑定）→ 2. SOP / 技能要求 → 3. 开工点检模板 → 4. 线边库 / 料口映射 → 5. 生产 BOM。

## 配置依赖概览

| 依赖 | 影响 | 在哪确认 |
| --- | --- | --- |
| DBC 产线 / 工位 / 工序 / 物料 | 客户端、SOP、技能、料口挂接对象 | DBC 工厂建模 / 物料 |
| 技能等级字典（DBC） | 工位技能要求能否校验上岗 | DBC |
| WMS 仓库 / 库位编码 | 线边库映射后能否协同库存 | WMS / 本页线边库 |
| 终端账号与登录方式 | 能否进入线边工作台 | 本页客户端 + 系统用户 |
