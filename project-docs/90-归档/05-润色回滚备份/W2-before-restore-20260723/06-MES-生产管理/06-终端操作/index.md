...
> 阅读对象：测试、实施（主）；线边班组长、操作工培训讲师、MES 实施（顺带）。

## 这一组解决什么问题 / 功能范围

终端操作说明 **MES 线边客户端** 上如何围绕已下发/已开工的运行工单完成：上下工、领取工位作业、开始作业、单件/批量报工、转序门闸，以及开工点检、并行组布局等现场差异。读完应能开「领得到活 / 报得了工 / 门闸放行」类验证场景。

**范围外：** WMS PDA 收发货不属于本组。工单状态机见[计划管理](../03-计划管理/index.md)；抢单/派单与转序策略见[工艺管理](../02-工艺管理/index.md)；客户端与 SOP 配置见[基础建模](../01-基础建模/index.md)。

## 如何使用本组文档（测试 / 实施）

| 你的目的 | 建议阅读 |
| --- | --- |
| 设计线边主路径与异常验证 | **本页**（主文档：主路径、关键判断） |
| 培训操作步骤或排查领不到活 | [终端操作-维护与查询参考](终端操作-维护与查询参考.md) |
| 改终端绑定或 SOP | [基础建模](../01-基础建模/index.md) |
| 查报工 / 上下工历史 | [报表统计](../05-报表统计/index.md) |

## 本组学习顺序

1. 登录客户端并上工 → 2. 领取/开始工位作业 → 3. 按任务模式报工 → 4. 看转序门闸 → 5. 并行组差异（若使用）。

## 配置依赖概览

| 依赖 | 影响 | 在哪确认 |
| --- | --- | --- |
| 线边客户端绑定工位 / 用户 | 能否登录、能领哪些作业 | [基础建模](../01-基础建模/index.md) |
| 工单已下发且可执行 | 现场能否开干 | [计划管理](../03-计划管理/index.md) |
| 任务模式 | 单件 vs 批量报工入口 | 计划管理工单 |
| 分派模式 / 转序门槛 | 领作业方式与能否转序 | [工艺管理](../02-工艺管理/index.md) |
| 技能 / 开工点检 | 上岗与开产拦截 | 基础建模 |
