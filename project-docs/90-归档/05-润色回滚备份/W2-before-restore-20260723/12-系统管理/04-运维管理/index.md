...
> 阅读对象：**测试、实施、运维（主）**。业务用户通常不直接用本页完成现场作业。售前无需深入。
...
| 本分组覆盖 | 不在本分组 |
| --- | --- |
| 运维菜单入口地图、风险与回滚口径 | 基础设施能力细则（链到对应叶页） |
| 与业务补偿的分工提示 | 用运维入口替代业务单据闭环 |
...
| 依赖 | 影响 |
| --- | --- |
| 运维角色与权限分离 | 高风险入口是否对业务角色开放 |
| 当前环境/租户上下文 | 误在生产执行清理/重试 |
| 变更窗口与备份 | 清理、重算、主配置切换风险 |
