...
> 阅读对象：**测试、实施（主）**；设备工程师、巡检保养执行人（顺带）。操作见[巡检保养-维护与查询参考](巡检保养-维护与查询参考.md)。售前停在[模块首页](../index.md)。
...
| 本分组覆盖 | 不在本分组 |
| --- | --- |
| 保养/巡检/点检（及可选开拉）计划→工单→记录 | QMS 过程质量巡检 ATR |
| 异常转报修出口 | MES 线边开工卡点细则；ANDON 呼叫主链 |
...
| --- | --- |
| 计划如何变成现场任务 | **本页** |
| 建计划、派工、执行、验证 | [巡检保养-维护与查询参考](巡检保养-维护与查询参考.md) |
| 配方案和检查项 | [基础数据](../01-基础数据/index.md) |
...
| 依赖 | 影响 |
| --- | --- |
| 方案与项启用、对象类型匹配 | 工单执行项是否为空 |
| 工作日历、周期/cron、提前天数 | 是否出工单（精确规则见 `GAP-016`） |
| 班组角色 | 派工无人 |
| 转报修开关/来源类型 | 不合格能否进入维修闭环 |
