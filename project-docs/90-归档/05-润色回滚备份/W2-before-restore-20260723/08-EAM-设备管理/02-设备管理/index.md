...
> 阅读对象：**测试、实施（主）**；设备工程师、维修班组（顺带）。操作见[设备管理-维护与查询参考](设备管理-维护与查询参考.md)。售前停在[模块首页](../index.md)。
...
| 本分组覆盖 | 不在本分组 |
| --- | --- |
| 到货验收、移动/变更、停机、报修→维修→双验证、维修备件入口 | DBC 台账身份主维护 |
| 巡检/保养等来源转入报修 | ANDON 呼叫主链；MES 工位作业 |
...
| 依赖 | 影响 |
| --- | --- |
| DBC 设备台账编码/可用态 | 能否报修、派工挂接 |
| 故障类型、班组角色 | 分类与可派对象 |
| 自动审核/派工/执行策略（环境） | 是否跳过人工环节（细节见 `GAP-016`） |
| 备件/WMS 通道 | 维修领料是否入账 |
