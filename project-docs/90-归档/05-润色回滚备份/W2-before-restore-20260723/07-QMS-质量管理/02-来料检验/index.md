...
> 阅读对象：测试、实施（主）；IQC、仓库协同、质量主管（顺带）。

## 这一组解决什么问题 / 功能范围

来料检验把供应商到货中需要质量确认的物料，转化为可追溯的检验申请、任务与记录，并给出接收/拒绝与使用决策。读完应能说清到货确认触发主链、免检跳过、发布回写边界。

**范围外：** 库存放行/隔离/退货等**事务以 WMS 为准**。收货主链见 WMS [采购收货](../../05-WMS-库房管理/03-采购收货/index.md)。采购收货记录侧「创建检验申请」入口**已停用**。

## 如何使用本组文档（测试 / 实施）

| 你的目的 | 建议阅读 |
| --- | --- |
| 设计到货→IQC→回写验证场景 | **本页**（主文档） |
| 处理申请/任务/记录操作与字段细节 | [来料检验-维护与查询参考](来料检验-维护与查询参考.md) |
| 维护抽样与方案 | [检验配置](../01-检验配置/index.md) |
| 不合格要评审或索赔 | [质量评审](../05-质量评审/index.md) |

## 本组学习顺序

1. 确认方案非免检 → 2. 到货确认待检触发建申请 → 3. 任务执行与判定 → 4. 记录发布与到货回写 → 5. 不合格出口（评审/通知）。

## 配置依赖概览

| 依赖 | 影响 | 在哪确认 |
| --- | --- | --- |
| 物料来料检验方案 / 免检 | 是否建单、抽什么 | [检验配置](../01-检验配置/index.md) |
| WMS 到货确认 + 库存待检 | **当前主触发点** | WMS 到货确认 |
| ASN / 采购订单行 | 追溯与回写键 | 上游单据 |
| 自动提交/同意/执行策略 | 申请推进节奏 | 本页申请字段 + 环境配置 |
...
- `FSEM-006`：方案/任务选择器精确状态过滤与 P13 投影矩阵待测。
