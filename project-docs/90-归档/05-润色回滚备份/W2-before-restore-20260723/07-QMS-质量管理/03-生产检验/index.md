...
> 阅读对象：测试、实施（主）；IPQC、生产质量、线边协同（顺带）。

## 这一组解决什么问题 / 功能范围

生产检验覆盖制造过程中的质量确认，菜单拆为四套申请—任务—记录：
...
本页写四套 ATR 的共同模型与边界。工艺节点线索见 MES [工艺管理](../../06-MES-生产管理/02-工艺管理/index.md)；工单执行见[计划管理](../../06-MES-生产管理/03-计划管理/index.md) / [终端操作](../../06-MES-生产管理/06-终端操作/index.md)。

**已证实触发：** WMS 制品收货可自动建检验（类型码与「巡检」枚举名相同的码）。**未证实：** MES 报工 NG 自动建 QMS 单（`GAP-071`）。

## 如何使用本组文档（测试 / 实施）

| 你的目的 | 建议阅读 |
| --- | --- |
| 设计四套生产检验分工与触发验证 | **本页**（主文档） |
| 做首件/末件/巡检/其他操作细节 | [生产检验-维护与查询参考](生产检验-维护与查询参考.md) |
| 配方案与工序码 | [检验配置](../01-检验配置/index.md) |

## 本组学习顺序

1. 按组织定义选对菜单类型 → 2. 确认对应类型方案 → 3. 手工或制品收货触发 → 4. ATR 执行与结论 → 5. 不合格进评审（若需要）。

## 配置依赖概览

| 依赖 | 影响 | 在哪确认 |
| --- | --- | --- |
| 对应检验类型的方案 | 抽什么、多严 | [检验配置](../01-检验配置/index.md) |
| WMS 制品收货 | 可自动建申请 | WMS 生产管理 / 制品收货 |
| 方案 MES 工序码 | 挂接线索 | 检验配置；自动建单 ❓ `GAP-071` |
| 手工申请权限 | 四套菜单均可手工建 | 权限配置 |
