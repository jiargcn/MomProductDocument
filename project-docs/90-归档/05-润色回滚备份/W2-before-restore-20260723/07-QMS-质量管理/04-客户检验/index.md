...
> 阅读对象：测试、实施（主）；客退质量、仓储退货协同（顺带）。

## 这一组解决什么问题 / 功能范围

本组菜单为**客户退回检验**（申请/任务/记录），用于客户退货入库后的质量确认。读完应能说清退货触发建单、与出货 OQC 的边界。

**不是「出货 OQC」专页。** 检验类型枚举虽存在发货检验等码，但 QMS 菜单未提供对应独立 ATR；出货质量协同应联查销售出库/SCP，勿在本页杜撰出货放行流程。退货仓储主链见 WMS；供应链协同见 SCP。
...
| --- | --- |
| 设计客退→检验验证场景 | **本页**（主文档） |
| 处理客退检验单操作细节 | [客户检验-维护与查询参考](客户检验-维护与查询参考.md) |
| 出货/发运问题 | WMS 销售出库；**勿默认本页** |
| 配客户退回检验方案 | [检验配置](../01-检验配置/index.md) |

## 本组学习顺序

1. 确认客户退回检验方案 → 2. WMS 客户退货记录触发（规则开关）或手工建申请 → 3. ATR 执行 → 4. 不合格出口（评审/通知）→ 5. 库存后续回 WMS。

## 配置依赖概览

| 依赖 | 影响 | 在哪确认 |
| --- | --- | --- |
| 客户退回检验方案 | 类型与抽样 | [检验配置](../01-检验配置/index.md) |
| WMS 客户退货 +「创建后建检验」规则 | 是否自动建申请 | WMS 客户退货 |
| 包装 / 批次 / 来源销售订单 | 追溯键 | 上游退货记录 |
| 寄售退货/发货调整等旁路 | 可能建检，类型需联查 | 环境规则 |
