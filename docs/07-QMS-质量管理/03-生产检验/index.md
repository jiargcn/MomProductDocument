...
| --- | --- | --- | --- | --- |
| 检验类型（首件/末件/巡检/其他） | P1 / P6 | 菜单与方案分流 | 四套 ATR 同结构、靠类型区分 | 选错菜单→方案/统计错 |
| 工单/产线/物料/批次 | P2 / P5 | 生产上下文 | 手工或上游带入 | 无上下文难追溯 |
| 触发：WMS 制品收货 | P9 / P12 | 自动建申请（已证实一类） | 类型码与「巡检」枚举名同码场景 | 与菜单名勿简单等同 |
| 触发：MES 报工 NG | P12 | **未证实**自动建单 | 仅作协同线索（`GAP-071`） | 勿写成固定自动建单 |
| 评估码 / 使用决策 | P1 / P9 | 判定与处置意图 | 同来料口径；库存归 WMS | 决策≠库存已变 |
| ATR 状态 | P9 | 门禁 | 同来料九态/四态 | — |
