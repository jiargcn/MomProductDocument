...
| --- | --- | --- | --- | --- |
| 物料 + 检验类型 | P2 / P6 | 方案业务键 | 同物料同类型有效期勿重叠 | 重叠导致执行选错方案 |
| 检验模板 | P2 / P12 | 步骤与特性来源 | 须引用已维护模板/过程 | 空模板无法执行 |
| 免检 | P1 / P12 | 是否跳过建申请 | 到货确认会查询免检 | 误免检漏检 |
| 自动创建申请 | P1 / P12 | 上游是否自动建单 | 与上游开关叠加 | 关了却以为会自动建 |
| AQL / 检验水平 | P1 / P7 | 决定抽多少 | 依赖样本字码与 AQL 行 | 水平错 → 抽样数错 |
| 改库存状态 | P1 / P12 | 配置意图开关 | **真正改库存仍以 WMS 为准** | 勿当已实现库存事务 |
| MES 工序码 | P2 / P12 | 过程检验挂接线索 | 自动触发映射 ❓（`GAP-071`） | 写成固定自动建单会误导 |
| 生效/失效日期 | P10 | 方案时间窗 | 过期不可被新申请引用（以校验为准） | 过期方案仍被手工误用 |
| 动态规则 / 计数器 | P1 / P12 | 加严/放宽 | 按物料+类型+供应商累计 | 层级选错加严不到位 |
| 选定集 | P1 | 定性可选值 | 缺陷等级来源 | 缺项导致无法判定 |
