...
> 阅读对象：测试、实施、运维（主）；IQC、仓库协同、质量主管等现场角色（顺带）；操作见[来料检验-维护与查询参考](来料检验-维护与查询参考.md)。

## 业务目的与适用范围

来料检验把供应商到货中需要质量确认的物料，转化为可追溯的检验申请、任务与记录，并给出接收/拒绝与使用决策结论。库存放行、隔离、退货等**事务以 WMS 为准**；本页只写检验链与触发/回写边界。读完本页，应能判断一批到货为何没生成检验申请、检验结论发布后如何回写到货侧数量。
