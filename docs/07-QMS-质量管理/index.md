...
QMS 把质量要求配成可执行的检验方案，并沉淀来料、生产过程、客户退回等场景的检验申请—任务—记录，以及不合格后的评审与质量通知。读完本页，应能立刻分清四层：

1. **检验配置**：规则主数据（方案/抽样/加严），不执行现场录入。
2. **三条执行链**：来料（到货确认触发）、生产（首件/末件/巡检/其他）、客户退回（客退检验，非出货 OQC）。
3. **质量评审**：不合格处置结论与通知单；≠ WMS「质量评审」菜单。
4. **库存事务**：隔离/放行/退货/报废等**写入以 WMS 为准**；使用决策只是意图。

报工细节归 MES；`GAP-071`（报工 NG→自动建检）未证实前勿当固定触发。通用 ATR 见[申请、任务与记录模型](../02-业务模型/01-申请任务记录模型.md)。
...
| 跟一笔到货/过程/客退检验 | 按学习顺序进对应分组（来料已定标可直接作对照） |
| 不合格闭环或索赔通知 | [质量评审](05-质量评审/index.md)；库存后续仍查 WMS |
| 排障：未建检、结论回写、不合格去向 | 先查边界表归属，再进分组维护参考 |
