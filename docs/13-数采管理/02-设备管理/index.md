...
> 阅读对象：测试、实施、运维（主）；设备/自动化工程师（顺带）。操作见[设备管理-维护与查询参考](设备管理-维护与查询参考.md)。

## 业务目的与适用范围

设备启用了、参数也打开了采集，日志却还是空的——这类问题需要按「设备 → 参数 → 边缘 → MQTT」一层层排查。本组覆盖数采侧的**设备主档、采集结果查询与 MQTT 链路监控**：

- **设备管理（采集数据资产）**：维护参与采集的设备编码、名称、类别与启用状态。
- **采集日志**：查询参数采集时序（设备、参数、采集点、值、客户端等）。
- **MQTT 客户端监控**：查看客户端在线状态与上下线时间。

名称中的「设备」指**数采资产**，与 DBC「设备台账」、EAM「维修对象」可能编码对齐，但页面与数据对象独立，**未证实**自动同步。读完本页，应能分清「这个设备」到底是数采资产、DBC 台账还是 EAM 维修对象，并知道无日志时先查哪一层。

## 如何使用本组文档

| 你的目的 | 建议阅读 |
| --- | --- |
| 想理解设备—日志—MQTT 关系 | 本页：准备 → 主路径 → 关键判断 → 验证点 |
| 弄清无数据时怎么分层排查 | 本页「写实示例」「建议验证点」 |
...
!!! example "写实示例：给定配置 → 期望行为"
    **给定：** 设备 `DEV-01` 已启用；参数已打开采集且分区对齐；MQTT 客户端在线并按约定上报。
    **期望：** 采集日志可按设备编码查到近期值；客户端 ID 可与 MQTT 监控对照。若设备未启用或 MQTT 长期离线，日志为空时先分层排除，不要先改 MES 点检单据。
...
- 新建并启用一台采集设备；为其配置至少一项「采集」参数。
- 边缘上报后，采集日志可按设备编码查到值。
- MQTT 监控可见对应客户端状态（环境启用 MQTT 时）；与日志客户端 ID 可对照。
- 导出日志/监控（有权限时）可用。

完整筛选与字段核对见[维护与查询参考](设备管理-维护与查询参考.md)。

## 限制与待确认

以下边界容易被当成已验证事实，实际仍待环境或联调核实：
