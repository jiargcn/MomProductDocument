...
设备启用了、参数也打开了采集，日志却还是空的——这类问题需要按「设备→参数→边缘→MQTT」一层层排查，本页就是这张排查图。本组覆盖数采侧的**设备主档、采集结果查询与 MQTT 链路监控**：

- **设备管理（采集数据资产）**：维护参与采集的设备编码、名称、类别与启用状态。
- **采集日志**：查询参数采集时序（设备、参数、采集点、值、客户端等）。
- **MQTT 客户端监控**：查看客户端在线状态与上下线时间。

名称中的「设备」指**数采资产**，与 DBC「设备台账」、EAM「维修对象」可能编码对齐，但页面与数据对象独立，**未证实**自动同步。读完本页，你能分清「这个设备」到底是数采资产、DBC 台账，还是 EAM 维修对象。

## 如何使用本组文档

设备建档、采集点配置、消费侧解读读数分属不同页，按目的挑一个看：
