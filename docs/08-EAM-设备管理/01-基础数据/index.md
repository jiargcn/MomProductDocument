...
通例见[通用选择器过滤惯例](../../02-业务模型/12-通用选择器过滤惯例.md)。本组多为配置侧选择器；身份台账「仅可选已存在 DBC 台账」。精确可选状态与权限范围见通例；未闭合项见辅证 `FSEM-006` / `GAP-014`。
...
- 方案/故障/班组/台账选择器的精确可选状态与权限投影范围尚未闭合；验收勿按「全状态可见」假定。（辅证：`FSEM-006`）
