...
巡检计划建不出来、报修单里挑不到想要的故障类型——多半不是流程坏了，而是本页要维护的配置没到位。基础数据是 EAM 的「配置底座」：方案、故障分类、班组角色、部件与备件基础、文档类型等都在这里定义，直接决定计划能否生成、报修能否分类、工单能派给谁。设备/工装**身份台账**以 DBC 为准（EAM 菜单会嵌入 DBC 台账页）；本页只写 EAM 模块内已落地的配置对象。读完本页，你能判断某个配置该在这里改，还是该回 DBC 台账改。

旧概述中的虚构「EAM_CONFIG / FACTORY_WORKSHOP」英文字段表废弃，不以之为培训事实。

## 如何使用本组文档

配置分散在本页、维护参考与 DBC 台账三处，对号入座能少走弯路：
