...
> 阅读对象：测试、实施、运维（主）；设备工程师、EAM 配置员（顺带）。操作见[基础数据-维护与查询参考](基础数据-维护与查询参考.md)。

## 业务目的与适用范围

巡检计划建不出来、报修单里挑不到故障类型——多半不是流程坏了，而是本页配置没到位。基础数据是 EAM 的配置底座：方案、故障分类、班组角色、部件与备件基础、文档类型等在此定义，直接决定计划能否生成、报修能否分类、工单能派给谁。

设备/工装**身份台账**以 DBC 为准（EAM 菜单会打开 DBC 台账页）；本页只写 EAM 侧已落地的配置对象，不替代台账主维护。读完本页，应能判断：某个缺口该在本页改配置，还是回 DBC 改身份，以及配错后下游会卡在哪一步。

旧概述中的虚构「EAM_CONFIG / FACTORY_WORKSHOP」英文字段表废弃，不以之为培训事实。

## 如何使用本组文档

配置分散在本页、维护参考与 DBC 台账三处，按目的对号入座：

| 你的目的 | 建议阅读 |
| --- | --- |
| 理解开计划/报修前要配什么、与 DBC 台账如何分界 | 本页：准备 → 对象关系 → 边界 → 写实示例 → 验证点 |
| 落地维护方案、故障类型、班组角色，或排查「选不到」 | [基础数据-维护与查询参考](基础数据-维护与查询参考.md) |
| 改设备/工装编号、车间、责任人等身份 | DBC [设备台账管理](../../04-DBC-主数据管理/07-设备管理/02-设备台账管理.md)（勿在本组硬改） |
| 看计划如何消费方案、异常如何转报修 | [巡检保养](../05-巡检保养/index.md)、[设备管理](../02-设备管理/index.md) |
...
!!! example "📝 示例数据占位"
    新建「月度保养方案」并挂三项检查项，供设备计划引用；再配一类故障类型与一条班组角色映射。

!!! example "写实示例：给定配置 → 期望行为"
    **给定：** 已启用巡检方案 OPT-WEEK（适用类型=设备，挂 3 项检查项）；故障类型 FT-SPINDLE 可用；班组角色「维修班」已映射到用户 U1；DBC 设备 EQ-001 可用。
    **期望：**

    1. [巡检保养](../05-巡检保养/index.md) 建计划时可选到 OPT-WEEK；方案停用后计划侧选不到该项。
    2. 报修可选 FT-SPINDLE；删除或停用已被历史引用的类型会影响新单分类（勿盲删）。
    3. 工单派工对象能落到 U1；映射为空时工单易停在待派工/待接单。
    4. 在 EAM「设备台账」菜单改型号/车间，实际打开的是 DBC 台账页；本页无法作为身份主维护入口。

### 建议验证点

- 方案启用/停用各走一次：计划能否选到、工单是否带出检查项。
- 故障类型缺失时新建报修：分类是否必选、选不到时的提示。
- 班组角色无用户映射时派工：是否无人可接。
- 故意在 EAM 基础数据页找「改设备型号」入口：应回到 DBC 台账，而不是在本组虚构字段改身份。
- 生产商/供应商（EAM 菜单）与 DBC 生产商对照：勿默认双改同一张主数据（同步 ❓）。
