...
EAM 负责设备与工装的**运行维护执行**：验收/移动/变更、停机、报修维修、巡检/保养/点检（及可选开拉）、备件业务与 EAM PDA。读完本页，你能判断某个设备/工装问题该进 EAM 哪个分组处理、还是该去 DBC 台账页找身份信息。设备/工装**身份台账**以 DBC 为准（侧栏「设备台账（DBC）」；EAM 导航常嵌入该台账页）。备件出入库可同步 WMS 非计划发料/收货，**库存余额以 WMS 为准**。
