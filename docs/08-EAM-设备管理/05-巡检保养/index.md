...
> 阅读对象：测试、实施、运维（主）；设备工程师、巡检保养执行人（顺带）。操作见[巡检保养-维护与查询参考](巡检保养-维护与查询参考.md)。

## 业务目的与适用范围

计划到了日子却不出工单、执行完却没转成报修——多半是排程或转出规则没配对。巡检保养把预防性维护落实为可排程的**计划 → 工单 → 记录**，覆盖保养、巡检、点检，以及可选的开拉；异常可转[报修维修](../02-设备管理/index.md)，方案与检查项在[基础数据](../01-基础数据/index.md)维护。

本页不写 QMS 过程巡检，也不写 MES 开工点检的线边细则（MES 仅有配置入口时，执行仍以 EAM 点检为准）。需要产线呼叫到岗时走 ANDON，不替代响应链。读完本页，应能判断「不出工单」查计划还是查方案，以及不合格何时转报修。

## 如何使用本组文档

| 你的目的 | 建议阅读 |
| --- | --- |
| 理解计划如何变成现场任务、异常如何出口 | 本页：准备 → 一次计划如何落到记录 → 写实示例 → 验证点 |
| 弄清不出工单/项空/转报修失败如何排查 | 本页「关键判断」「建议验证点」 |
| 建计划、派工、执行、验证的操作细节 | [巡检保养-维护与查询参考](巡检保养-维护与查询参考.md) |
| 配方案和检查项 / PDA 执行 | [基础数据](../01-基础数据/index.md)、[终端操作](../06-终端操作/index.md) |
...
!!! example "写实示例：给定配置 → 期望行为"
    **给定：** 设备 EQ-001；周巡检方案 OPT-WEEK（3 项，其中 1 项可判定不合格）；计划已审批，周期/提前天数可使当期出单；班组角色已映射执行人；组织启用「异常转报修」。
    **期望：**

    1. 到生成窗口后出现巡检工单，带出方案项；方案停用或项未挂时工单项为空或无法建计划。
    2. 接单→执行→提交→验证→记录；未验证不得当完成。
    3. 一项不合格转报修：报修带来源工单类型/号；该报修进入 EAM 维修链，**不是** QMS 巡检检验，也**不是** ANDON 呼叫本身。
    4. 若还需产线到岗，按组织流程另开 ANDON；EAM 不替代响应链。

### 建议验证点

- 方案+计划审批后能生成至少一张工单；停用方案后新周期不再带出该项。
- 工单走通接单→执行→验证→记录；逾期/作废路径可演示（权限允许时）。
- 项空：核对方案编码与项关联。
- 不合格转报修：来源可追溯；勿与 QMS「巡检检验」菜单混淆。
- PDA 与 Web 同一工单状态一致（抽查一条）。
- 定时生成/日历槽位精确规则以环境为准（`GAP-016`），培训勿写死「到点必出」。
