...
工装台账改不了、巡检计划选不到工装，通常是走错了模块。工装管理覆盖工装验收、出入库记录、变更与改制等执行履历；工装**台账身份**在 DBC（EAM 菜单「工装台账管理」打开 DBC 页）。读完本页，你能分清「身份改哪、履历记哪」；巡检/保养/点检/维修对工装与设备共用执行模型，见[巡检保养](../05-巡检保养/index.md)、[终端操作](../06-终端操作/index.md)。

## 如何使用本组文档

台账、履历、预防维护分在不同页维护，对照下表能少绕路：
