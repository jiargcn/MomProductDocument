...
> 阅读对象：测试、实施、运维（主）；设备工程师、维修班组、生产协同（顺带）。操作见[设备管理-维护与查询参考](设备管理-维护与查询参考.md)。

## 业务目的与适用范围

设备坏了该报修给谁、维修单卡在哪一步该催谁——答案在设备管理。它覆盖验收投用、移动/变更、停机登记，以及报修→维修→双验证闭环。设备**台账身份**在 DBC 维护（EAM「设备台账管理」实际打开 DBC 台账页）；本页写 EAM **执行侧**已证实能力。

与 [ANDON](../../09-ANDON-异常管理/index.md) 的边界要分清：ANDON 管产线呼叫与到岗响应；本页管报修审批与维修工单状态机。二者可并行引用同一设备编码，但**呼叫时长 ≠ 维修工时**，停机项目也不直接等同。不替代 MES 工位作业。旧虚构 ER 字段废弃。

读完本页，应能跟完一笔报修闭环，并判断问题该回 DBC 改台账、在本页跟维修，还是去 ANDON 查呼叫。

## 如何使用本组文档

| 你的目的 | 建议阅读 |
| --- | --- |
| 理解台账 vs 维修、报修闭环主线 | 本页：准备 → 一次报修如何闭环 → 写实示例 → 边界 → 验证点 |
| 弄清状态门禁或据此验证/排障 | 本页「报修与维修状态」「关键判断」「建议验证点」 |
| 验收、移机、报修、派工、验证的操作细节 | [设备管理-维护与查询参考](设备管理-维护与查询参考.md) |
| 改设备编号/车间/责任人 | DBC [设备台账管理](../../04-DBC-主数据管理/07-设备管理/02-设备台账管理.md) |
| 巡检异常转报修 / PDA 现场执行 | [巡检保养](../05-巡检保养/index.md)、[终端操作](../06-终端操作/index.md) |
| 产线呼叫与到岗（非维修工单） | ANDON [故障记录](../../09-ANDON-异常管理/01-故障记录/index.md) |
...
!!! example "📝 示例数据占位"
    设备 EQ-001 报修「主轴异响」→ 派工 → 更换备件 → 双验证完成；可并列一条 ANDON 呼叫仅作对照（勿混时长）。

!!! example "写实示例：给定配置 → 期望行为"
    **给定：** DBC 设备 EQ-001 可用；故障类型 FT-SPINDLE；报修紧急度=高；组织未开自动审核；班组角色已映射维修人 M1；现场另有 ANDON 呼叫记录（同设备）。
    **期望：**

    1. 报修须选已存在台账编码；台账停用时通常选不到或不可推进（精确过滤 ❓）。
    2. 提交后停在待审核/审核中，未审通过前不应假定已生成维修工单。
    3. 通过后生成维修：待派工 → M1 接单 → 执行 → 维修验证 → 报修验证 → 已完成；跳过验证不得当完成。
    4. ANDON 呼叫可并存，但结束呼叫不等于维修完成；停机事实与 ANDON 停机项目勿直接等同（自动关联 ❓）。
    5. 改设备车间/责任人须打开 DBC 台账页，不在报修单上“改身份”。

### 建议验证点

- 未建台账编码报修：选择器应拦或无法保存。
- 报修审批驳回后再提交：状态与维修是否未误生成。
- 工单走通接单→执行→双验证；尝试跳过验证是否被拦。
- 巡检异常转报修：来源工单类型/号可追溯（若启用）。
- 同设备已有 ANDON 呼叫时开报修：确认两套单据可并存且时长口径不混用。
- 维修领用备件：联查备件出库与 WMS 同步状态（见[备件管理](../03-备件管理/index.md)）。

## 与 DBC / ANDON / MES 边界

报修单该开在哪个系统、停机记录算谁的，是这条边界上最常被问到的问题：
