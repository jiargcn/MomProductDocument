...
> 阅读对象：测试、实施、运维（主）；备件管理员、维修班组、仓库协同（顺带）。操作见[备件管理-维护与查询参考](备件管理-维护与查询参考.md)。

## 业务目的与适用范围

维修领了一颗轴承，EAM 台账数量和 WMS 却对不上——根子往往在于没搞清「谁是库存权威」。备件管理支撑维修/保养所需物料的台账视图、申领、出入库、盘点与工单备件执行；EAM 维护业务单据与模块内结存视图，**向 WMS 同步非计划出库/入库**已证实存在，但库存余额与仓库事务始终以 WMS 为准。

读完本页，应能判断对账查哪一边、同步失败先查什么，以及工单领料与独立领用为何不能混菜单。

## 如何使用本组文档

| 你的目的 | 建议阅读 |
| --- | --- |
| 理解 EAM 备件视图 vs WMS 库存权威 | 本页：准备 → 对象关系 → WMS 边界 → 写实示例 → 验证点 |
| 弄清同步失败/对账差异如何排查 | 本页「关键判断」「建议验证点」与维护参考「WMS 同步」 |
| 申领、出库、入库、盘点、工单领料操作 | [备件管理-维护与查询参考](备件管理-维护与查询参考.md) |
| 维修领用入口 / WMS 余额查询 | [设备管理](../02-设备管理/index.md)、WMS [库存管理](../../05-WMS-库房管理/09-库存管理/index.md) |
...
!!! example "📝 示例数据占位"
    维修工单领用轴承 → 出库同步 WMS 非计划发料 → 回写单号；另备一例同步失败状态对照。

!!! example "写实示例：给定配置 → 期望行为"
    **给定：** 维修工单 J-9001 执行中；备件 SP-BRG 有 EAM 台账结存；EAM 库位与 WMS 映射正确；账期开放；走工单备件出库 2 件。
    **期望：**

    1. 出库成功后同步状态为已同步，并可回写 WMS 非计划发料单号；WMS 余额减少 2。
    2. 映射缺失或账期关闭时同步失败：EAM 侧可见失败状态，不得只改 EAM 数量“抹平”，也勿盲开第二张 WMS 单。
    3. 独立领用与工单备件是不同菜单路径；漏绑工单会导致维修用料难追溯。
    4. 对账以 WMS 余额与事务为准；EAM 台账是业务视图。

### 建议验证点

- 工单备件出库成功：核对同步状态、WMS 单号与余额减少。
- 故意错映射或关账期：确认失败可复现且状态可见。
- 独立领用与工单领料各走一笔：路径与追溯字段不同。
- 同步失败后禁止只改 EAM 数量；重试或纠错后再对账。
- 盘点调整是否回写 WMS：记录实测结果（完整链路 ❓）。
