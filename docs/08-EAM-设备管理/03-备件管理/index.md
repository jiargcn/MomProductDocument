...
通例见[通用选择器过滤惯例](../../02-业务模型/12-通用选择器过滤惯例.md)。备件编码来自备件基础/台账；去向设备「仅可选已存在台账」。精确可选状态与权限范围见通例；未闭合项见辅证 `FSEM-006` / `GAP-014`。
...
- 备件/库位/工单/设备选择器的精确可选状态与权限投影范围尚未闭合；验收勿按「全状态可见」假定。（辅证：`FSEM-006`）
!!! example "📝 示例数据占位"
