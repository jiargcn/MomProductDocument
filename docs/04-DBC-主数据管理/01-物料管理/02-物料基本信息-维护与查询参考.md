...
**模式归类：** 本页以 **P1（用途/属性开关）**、**P2（字典选择）**、**P6（物料号）**、**P10（有效天数）**、**P14（审计）** 为主。

**本页无 P3 / P4 / P5 / P7 / P8 / P9 / P11 / P12：** 无库存余额选择、无仓区位级联、无来源单回填、无收发数量换算、无现场采集门禁、无单据状态机；默认包装/默认库位/策略挂接见物料包装、物料库区配置及策略页。

### 字段业务语义总表
| 字段（中文业务名） | 行为模式 | 业务作用 | 谁在何时维护/选择 | 业务约束摘要 | 影响哪些功能 | 变更或错选风险 |
| --- | --- | --- | --- | --- | --- | --- |
| 物料号 | P6 | 唯一业务识别 | 主数据维护员新增时录入 | 必填、不重复、Web 编辑锁定；最长 30 字符 | 所有引用物料的单据与主数据关系 | 改码/重复导致引用断裂或双码（`GAP-013`/`GAP-006`） |
...
| 替代单位 | P2/P7 | 换算用辅助单位 | 按需选择；导入层必填口径不一致 | ❓ 换算关系维护位置与强制规则待确认（`GAP-018`） | 需双单位的收发场景 | 导入失败或现场换算混乱 |
| 可采购 | P1 | 是否走采购获取 | 新增/修改用途开关 | 取值是/否；❓ 选择器过滤时点待确认 | 采购订单、采购收货、供应商物料匹配 | 误关导致无法采购闭环 |
...
| 是否可用 | P1 | 业务可用性 | 启用/禁用 | 默认可用；停用过滤 ❓ 待确认 | 各业务物料选择器 | 停用后仍被引用的在途单 |
...
| --- | --- | --- | --- | --- | --- |
| 物料类型 | 物料类型字典 | 系统已配置的物料类型项 | 字典是否启用；❓ 组织差异待确认 | 可能约束有效天数上限等管控项展示 | 字典未维护、项已停用 |
| 基本单位 / 替代单位 | 计量单位字典 | 已配置的计量单位 | ❓ 是否按物料类型再裁剪待确认 | 作为后续收发计量基础；替代单位换算规则 ❓（`GAP-018`） | 单位未建档、导入口径与页面不一致 |
