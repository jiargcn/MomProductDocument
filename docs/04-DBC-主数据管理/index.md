...
> 阅读对象：测试、实施、运维（主）；主数据维护、采购/销售协同、仓储与生产管理人员（顺带）。

## 模块解决什么问题

DBC 维护各业务共用的**基础资料与运行配置**（物料/伙伴/地点/工艺/策略等），让下游开单时「选得到、对得上、说得清」。读完本页，应能：判断一项资料该在 DBC 哪组维护；下游选不到对象时先查状态/用途/归属还是策略；并分清 DBC 管身份与配置、业务模块管执行结果。

不替代 WMS/MES/QMS 等现场作业。标签打印可在业务页触发，模板与设备长期归[基础设施](../03-基础设施/01-标签、条码与打印.md)。**与 EAM：** [设备台账（DBC）](07-设备管理/index.md)管身份；报修/巡检/备件执行归[EAM](../08-EAM-设备管理/index.md)。

## 如何使用本模块

| 你的目的 | 建议怎么做 |
| --- | --- |
| 理解 DBC 整体定位与和下游模块的协作关系 | 本页：模块解决什么问题 → DBC 在业务中的位置。 |
| 新增、维护、停用物料/供应商/客户/工厂模型/业务配置（主数据维护、实施） | 按下方“建议学习与维护顺序”补齐基础资料，再进入对应业务分组的主文档维护。 |
| 排障/答疑：某物料、伙伴、库位或配置为何选不到、为何这样生效 | 本页“常见问题与处理”；具体字段行为回对应分组主文档查“关键字段业务角色”。 |
| 进入具体分组细读维护方法 | 见下方“业务分组与学习入口”，按建议顺序进入分组 index，再进主文档。 |
...
!!! example "📝 示例数据占位"
    以“螺栓 M8×20”为例，展示物料、供应商物料、包装、默认库区和采购收货如何共同支撑一笔到货业务。
...
!!! example "📷 截图占位"
    DBC 菜单全景。标出物料管理、工厂建模、策略设置和物流配置的入口，使用脱敏测试环境截图。
