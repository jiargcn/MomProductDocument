...
MOM 以**多业务模块 + 统一系统/基础设施能力**组织：管理端与多终端接入，业务服务按域拆分，主数据与权限集中治理，库存/质量/执行等权威落在对应业务模块。技术栈细节见[技术栈](tech-stack.md)。
...
| 数据层 | PostgreSQL 业务库、Redis 缓存；报表与调度有独立组件 | [技术栈](tech-stack.md) |
...
模块级协作示意见上图与[集成场景](../14-API参考/03-跨系统集成场景.md)。**运行时服务拆分实例名、端口与注册中心**以部署环境为准，见[部署架构](../03-基础设施/deployment.md)待确认项。
