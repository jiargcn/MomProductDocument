...
本页不重复解释申请、任务、记录三类对象的通用设计目的。通用模型见[申请、任务与记录模型](../../02-业务模型/01-申请任务记录模型.md)。本页只说明采购收货的业务特性：采购订单/ASN 来源、供应商到货、收货任务执行、预计入生成、记录过账、库存事务、SAP/采购订单回写，以及 PDA 扫码收货和拒收入口。
...
当前待测试验证项包括：实际状态流转、审批/自动通过策略、PDA 与线边端的具体入口，以及接口调用是否在测试环境成功闭环。相关问题已登记在[产品差距总账](../../15-版本路线图/03-问题与待确认/产品差距总账.md)。
...
> 当前前端详情页分组能力尚不完整。本节先作为目标规划，用于指导后续页面改造、截图采集和培训材料编写。

#### 9.1 详情页分组

| 页面 | 建议分组 | 分组内容 | 目的 |
| --- | --- | --- | --- |
| 采购收货申请详情 | 基本信息 | 单据号、状态、业务类型、供应商、来源类型、部门、创建人、创建时间。 | 先让用户判断这张申请是谁提出、当前处于什么阶段。 |
| 采购收货申请详情 | 来源信息 | 采购订单号、订单行、ASN/发货单号、要货计划、订单类型、ERP 订单类型。 | 解释申请来自哪个上游业务对象。 |
| 采购收货申请详情 | 收货明细 | 物料、数量、采购计量数量、换算率、库存状态、库位、批次、包装号。 | 解释后续任务和预计入的明细来源。 |
| 采购收货申请详情 | 标签与接口 | 标签状态、标签生成记录、接口来源、接口推送标识。 | 解释标签重打和外部系统挂接。 |
| 采购收货任务详情 | 执行信息 | 任务号、申请单号、状态、承接人、承接时间、执行限制开关。 | 说明谁在执行、允许修改哪些内容。 |
| 采购收货任务详情 | 任务明细 | 物料、数量、库位、库存状态、批次、包装、扫描/执行数量。 | 支撑现场执行和差异分析。 |
| 采购收货任务详情 | 预计库存 | 预计入记录、任务号、业务类型、数量、库位、库存状态。 | 解释任务对预计库存的影响。 |
| 采购收货记录详情 | 记录信息 | 记录号、申请单号、任务单号、状态、事务类型、接口类型、ERP 凭证号。 | 说明已经发生的业务事实和接口结果。 |
| 采购收货记录详情 | 库存过账 | 库存事务、库存余额影响、撤销事务、冲抵记录。 | 解释库存变化来源。 |
| 采购收货记录详情 | 后续业务 | 上架申请、检验申请、采购退货、撤销记录。 | 让用户从收货结果继续追溯下游处理。 |

#### 9.2 快速跳转

| 起点 | 建议跳转目标 | 传参/过滤条件 | 用途 |
| --- | --- | --- | --- |
| 采购收货申请 | 采购收货任务 | `request_number = 当前申请单号` | 查看申请处理后生成了哪些任务。 |
| 采购收货申请 | 预计入库存 | `job_number in 当前申请关联任务号` | 查看任务阶段产生的预计入。 |
| 采购收货申请 | 采购订单/要货计划/ASN | `po_number`、`pp_number`、`asn_number` | 回查上游来源。 |
| 采购收货任务 | 采购收货申请 | `number = request_number` | 回查任务来源。 |
| 采购收货任务 | 预计入库存 | `job_number = 当前任务号` | 查看任务占用的预计入。 |
| 采购收货任务 | PDA 执行记录或任务详情 | `id`、`number` | 对照现场扫码执行情况。 |
| 采购收货记录 | 采购收货申请 | `number = request_number` | 回查业务意图。 |
| 采购收货记录 | 采购收货任务 | `number = job_number` | 回查现场执行任务。 |
| 采购收货记录 | 库存事务 | `record_number = 当前记录号` | 查看实际库存过账。 |
| 采购收货记录 | 库存余额 | 物料、批次、包装号、库存状态、库位、货主等库存粒度 | 查看收货后当前余额。 |
| 采购收货记录 | 上架/检验/采购退货 | 记录明细中的后续单号 | 追踪收货后的业务闭环。 |

### 10. 终端执行入口（PDA 第一轮取证）

| 入口 | 代码路径 | 当前证据 | 文档结论 |
| --- | --- | --- | --- |
| 采购收货任务列表 | `sourcecode/frontend/pda/src/pages/purchaseReceipt/job/purchaseReceiptJob.vue` | PDA 列表查询待处理、进行中任务；支持按任务号/ASN 扫描定位任务。 | PDA 是采购收货任务执行入口之一。 |
| 采购收货拒收任务列表 | `sourcecode/frontend/pda/src/pages/purchaseReceipt/job/purchaseRejectJob.vue` | 与收货任务列表类似，但进入详情时传入 `operation=reject`。 | 拒收在 PDA 中作为独立操作入口呈现。 |
| 采购收货任务详情 | `sourcecode/frontend/pda/src/pages/purchaseReceipt/job/receiptDetail.vue` | 进入待处理任务详情时自动承接；收货模式支持扫描包装标签、扫描/校验收货库位、全单收货开关、提交任务；拒收模式要求录入拒收原因。 | 任务执行的关键培训内容应放在采购收货页“终端操作”段，而不是单独建模块。 |
| PDA 菜单路由 | `sourcecode/frontend/pda/src/pages.json` | 存在 `pages/purchaseReceipt/job/purchaseReceiptJob`、`purchaseRejectJob`、`receiptDetail`、`record/purchaseReceiptCheck` 路由。 | 后续截图采集应覆盖任务列表、收货详情、拒收详情、记录检查。 |

PDA 执行特有规则：

- 待处理任务进入详情时会调用承接接口，任务进入进行中。
- 收货模式支持“全单收货”开关，但实际可修改数量仍受任务配置 `allowModifyQty` 影响。
- 若按包装管理，当前 PDA 逻辑会禁止直接修改数量。
- 是否必须扫描目标库位、是否允许修改目标库位，受任务配置 `allowScanTargetLocation`、`allowModifyLocation` 影响。
- 拒收模式必须填写拒收原因。
...
```mermaid
flowchart LR
    A["采购订单 / ASN / 要货计划"] --> B["采购收货申请<br/>request_purchasereceipt_main"]
    B --> C["处理申请<br/>自动或人工"]
    C --> D["采购收货任务<br/>job_purchasereceipt_main"]
    D --> E["预计入库存<br/>transaction_expectin"]
    D --> F["PDA / Web 执行收货"]
    F --> G["采购收货记录<br/>record_purchasereceipt_main"]
    G --> H["库存事务<br/>transaction_transaction"]
    H --> I["库存余额<br/>transaction_balance"]
    G --> J["上架 / 检验 / 采购退货 / SAP 回写"]
```
...
| 示例 | 业务含义 | 待填数据 | 来源 |
| --- | --- | --- | --- |
| 正常收货一笔 | 从采购订单生成申请，处理为任务，PDA 收货后生成记录、事务和余额。 | 采购订单号、申请单号、任务单号、记录号、事务号、物料、数量、库位、批次、包装号。 | 待测试环境构造。 |
| 少收一笔 | 实收数量小于申请/任务数量。 | 申请数量、实收数量、短缺数量、任务/申请状态变化。 | 待测试环境构造。 |
| 拒收一笔 | PDA 或 Web 拒收任务，生成拒收记录并删除预计入。 | 任务号、拒收原因、拒收记录号、预计入删除结果。 | 待测试环境构造。 |
| 撤销收货一笔 | 正常记录撤销，生成撤销事务和冲抵记录。 | 原记录号、撤销记录号、撤销事务号、余额前后数量、SAP/采购订单回退结果。 | 待测试环境构造。 |

#### 11.4 截图占位

| 截图 | 目的 | 状态 |
| --- | --- | --- |
| Web 采购收货申请列表 | 展示列表字段顺序、查询字段、状态和动作按钮。 | 占位，待测试环境截图。 |
| Web 采购收货申请详情 | 展示采购订单选择器、供应商带出、只读字段和业务类型默认策略。 | 占位，待测试环境截图。 |
| Web 采购收货任务列表 | 展示承接、关闭、放弃、拒收、撤销等动作状态。 | 占位，待测试环境截图。 |
| Web 采购收货记录列表 | 展示记录撤销、ERP 凭证号、接口推送状态。 | 占位，待测试环境截图。 |
| PDA 采购收货任务列表 | 展示按任务/ASN 扫描定位任务。 | 占位，待测试环境截图。 |
| PDA 收货详情 | 展示扫描包装、目标库位、全单收货、数量限制。 | 占位，待测试环境截图。 |
| PDA 拒收详情 | 展示拒收原因输入与提交。 | 占位，待测试环境截图。 |
