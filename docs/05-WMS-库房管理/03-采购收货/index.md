...
完整选择器范围、联动、回填与采集表见[维护与查询参考](01-采购收货-维护与查询参考.md)。本表只列主线关键项。

| 字段/配置点 | 行为模式 | 在系统中的作用 | 关键行为要点（取值/范围/联动/门禁） | 维护或操作时要警惕什么 |
| --- | --- | --- | --- | --- |
| 来源采购订单 / 送货通知等 | P2 / P5 | 确定本次到货范围并带回明细 | 订单须已发布且符合当前 ERP 订单类型；选中后回填供应商与明细 | 选错来源会带入错误物料与数量 |
| 供应商 | P2 / P5 | 到货责任方与核对基础 | 通常由来源单回填并锁定；❓ 是否允许手工改待确认 | 与实物送货方不一致会导致追溯错位 |
| 业务类型 | P1 / P12 | 采购收货场景的策略入口 | 按采购收货场景使用，不应随意替换 | 错选可能改变自动动作/编号/事务路径（❓ 细节待确认） |
| 明细物料 | P2 / P5 | 本次应收的物料清单 | 来自来源单明细；应可用且符合采购用途等约束（❓ 用途开关过滤待确认） | 选不到时先查来源明细与物料可用性 |
| 计划/实收数量与单位 | P7 | 核对到货量并记账 | 库存计量与采购计量可能并存；转换率 ❓ 待专项核验 | 超收/欠收受任务配置约束 |
| 库位 / 库存状态 | P2 / P4 / P8 | 收货落点与后续可用范围 | 受仓库/库区级联与任务扫描配置约束 | 扫错库位导致库存位置错误 |
| 批次 / 包装 / 托盘 | P8 / P3 | 库存唯一粒度维度 | 按任务要求采集；构成余额业务键的一部分，见[库存管理精度与唯一粒度](../../02-业务模型/08-库存管理精度与唯一粒度.md) | 漏采导致无法完成或追溯粒度不足 |
| 申请/任务/记录状态 | P9 | 门禁：某状态允许哪些动作 | 见下文「状态与关键动作」；具体状态码 ❓ 待测试确认 | 勿在非预期状态强行撤销（`GAP-009`） |
