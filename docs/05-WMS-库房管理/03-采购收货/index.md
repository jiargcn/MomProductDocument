...
> 阅读对象：测试、实施、运维（主，负责验证场景设计、配置落地与日常问题答疑/排障）；采购协同人员、仓库收货人员、现场执行人员、质量协同人员及需要追溯入库结果的业务人员（顺带受益）。

## 业务目的与适用范围

采购收货把“供应商送达的物料”转化为可追溯的内部收货结果。它连接采购订单或送货通知、仓库现场执行、库存变化，以及后续的上架、检验、退货和外部系统回写。

本页说明从到货到收货完成的一条业务链。读完本页，你能说清收货的完整链路和角色分工、哪些配置会改变现场可选范围与操作行为、遇到异常时先查什么、以及可以据此验证或排障的关键点。通用的申请、任务和记录概念见[申请、任务与记录模型](../../02-业务模型/01-申请任务记录模型.md)；本页只说明采购收货的来源、现场执行、库存影响和异常处理。
...
| 设计验证场景、讲清配置为何会改变现场行为，或据此现场答疑/排障 | 本页「配置如何起作用」「建议验证点」「常见问题与处理」。 |
| 正在发起申请、承接任务、使用 PDA、撤销或追溯一笔收货 | [采购收货-维护与查询参考](01-采购收货-维护与查询参考.md)。该页按申请、任务、记录和查询任务展开。 |
| 需要对照共享模型（申请/任务/记录、库存挂接、粒度） | [申请、任务与记录模型](../../02-业务模型/01-申请任务记录模型.md)、[库存数据挂接模型](../../02-业务模型/02-库存数据挂接模型.md)、[库存管理精度与唯一粒度](../../02-业务模型/08-库存管理精度与唯一粒度.md)。 |
...
!!! example "写实示例：给定配置 → 期望行为"
    **给定：** 采购订单 PO-1001 已发布、物料 A 计划 100 件；对应业务类型下发的任务配置为「允许少收」「必须扫描包装与库位」；现场实收 98、拒收 2。
    **期望：**
...
完整选择器范围、联动、回填与采集表见[维护与查询参考](01-采购收货-维护与查询参考.md)。本表只列主线关键项。订单/供应商/物料选择通例见[通用选择器过滤惯例](../../02-业务模型/12-通用选择器过滤惯例.md)；仓→区→位级联通例见[库位与仓储级联惯例](../../02-业务模型/13-库位与仓储级联惯例.md)。
...
| 批次 / 包装 / 托盘 | 库存唯一粒度维度 | 按任务要求采集；构成余额业务键的一部分，见[库存管理精度与唯一粒度](../../02-业务模型/08-库存管理精度与唯一粒度.md) | 漏采导致无法完成或追溯粒度不足 |
| 申请/任务/记录状态 | 门禁：某状态允许哪些动作 | 见下文「状态与关键动作」；具体状态码 ❓ 待测试确认 | 勿在非预期状态强行撤销（`GAP-009`） |

## 配置如何起作用

采购收货本身没有独立的策略配置页面；申请、任务、记录的可选范围与现场行为，主要受以下上游配置约束：

| 配置项 | 对采购收货的影响 | 维护入口 |
| --- | --- | --- |
| 业务类型 | 决定本次收货可选的物料/库存/地点范围、入出库方向，以及是否自动提交、自动同意、自动处理；同时通过「任务控制类开关」决定现场任务能否改数量、改库位、连续扫描等（该开关被采购收货任务读取）。 | [业务类型](../../04-DBC-主数据管理/05-策略设置/03-业务类型.md) |
| 单据设置 | 决定收货申请、任务、记录三类单据号的编号规则（前缀、日期段、流水长度、重置周期）；不改变收货的业务逻辑本身。 | [单据设置](../../04-DBC-主数据管理/05-策略设置/04-单据设置.md) |
| 单据开关 | 可能控制某项收货相关能力的启用或关闭；具体哪些开关代码作用于采购收货、是否即时生效，尚未逐项验证。❓ | [单据开关](../../04-DBC-主数据管理/05-策略设置/05-单据开关.md) |
| 任务扫描 / 现场控制配置 | 决定现场是否必须扫描包装、批次、库位，是否允许修改数量/库位/批次/包装号，是否允许全单收货、少收、多收或重复扫码。当前证据显示这类控制随任务（关联业务类型）下发；是否存在独立可视化配置入口尚未确认。❓ | 见上方「业务类型」；独立配置页 ❓ 待确认 |

变更以上任一配置前，应在测试环境用一笔完整收货（申请 → 任务 → 记录）走通再上生产，并保留回退路径（业务类型变更的通用验证顺序参考[业务类型「建议验证点」](../../04-DBC-主数据管理/05-策略设置/03-业务类型.md#建议验证点)）。

## 建议验证点

以下为供测试、实施、运维参考的短列表，可据此开单验证或排障定位，不是完整测试用例集：

- 更换业务类型后，重新走一遍申请 → 任务 → 记录，确认可选物料/库位范围、自动提交/自动处理路径是否符合该业务类型预期。
- 分别在「允许修改数量/库位/批次/包装」开启与关闭两种任务配置下验证现场收货、少收、多收、拒收的实际表现是否一致。
- 验证单据设置的编号规则在收货申请、任务、记录三类单据上是否都按预期生成（前缀、日期段、流水、重置周期）。
- 验证收货记录撤销后，库存事务、库存余额、预计入库及外部系统回写是否同步回退（对应 `GAP-009`）。
- 验证 Web 与 PDA 两个执行入口的任务列表、承接、执行、拒收表现是否一致（对应 `GAP-010`：Web 任务列表执行入口可能不可用）。
- 运维现场排障时，若反馈「无法修改数量/库位」或「扫码校验失败」，优先核对该任务所属业务类型的任务控制类开关，而非直接怀疑权限或系统故障。
...
采购收货复用[申请、任务与记录模型](../../02-业务模型/01-申请任务记录模型.md)的通用状态码。下图只画本业务已写明的动作与已知差异；自动策略跳转、按钮按角色可见性、撤销前置状态（`GAP-009`）仍以测试环境为准，图中用虚线标注。
...
stateDiagram-v2
    ...
    note right of 任务进行中
      Web列表执行入口可能不可用 GAP-010
      任务撤销可见性与后端校验一致性待测 GAP-009
    end note
```

库存挂接详见下文「对库存和相关业务的影响」，公共模型见[库存数据挂接模型](../../02-业务模型/02-库存数据挂接模型.md)。
