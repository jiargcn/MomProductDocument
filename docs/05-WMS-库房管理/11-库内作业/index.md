...
库内作业处理库存仍在企业内部、但地点、状态或数量需要受控变化的场景，例如调拨、库内转移、库存状态转移、计划外出入库和报废。它的目标是让每一次内部变化都有来源、执行过程和结果记录，避免直接改余额造成无法追溯的账实差异。

本页说明库内作业的共同业务逻辑与分类边界。通用的申请、任务和记录概念见[申请、任务与记录模型](../../02-业务模型/01-申请任务记录模型.md)。各子业务的字段、审批、状态和扫码规则不能互相套用；盘点差异应走[盘点管理](../12-盘点管理/index.md)，不应以库内作业替代。

## 如何使用本组文档

| 你的目的 | 建议阅读 |
| --- | --- |
| 想理解内部库存为何以及如何受控变化 | 本页。按作业分类、关键判断和库存影响理解主线。 |
| 正在发起申请、承接任务、使用 PDA 或追溯一笔库内变化 | [库内作业-维护与查询参考](库内作业-维护与查询参考.md)。 |
| 需要核对字段技术名或服务链路 | 由文档维护人员查询内部证据底稿；业务读者不需要阅读。 |
...
完整选择器与分类型差异见[维护与查询参考](库内作业-维护与查询参考.md)。多数调拨/库内转移/状态转移共用转移对象族，再按**业务类型**区分；计划外与报废为独立链。盘点差异应走[盘点管理](../12-盘点管理/index.md)。

| 字段/配置点 | 行为模式 | 在系统中的作用 | 关键行为要点（取值/范围/联动/门禁） | 维护或操作时要警惕什么 |
| --- | --- | --- | --- | --- |
| 作业类型 / 业务类型 | P1 / P12 | 决定菜单路径与库存动作 | 地点变化 vs 状态变化 vs 计划外数量 vs 报废 | 选错类型会走错审批与事务 |
| 来源库存余额 | P3 | 明确“从哪一笔库存变” | 按唯一粒度选择且可用量足够，见[库存管理精度与唯一粒度](../../02-业务模型/08-库存管理精度与唯一粒度.md) | 找不到可移动库存时先查状态/冻结 |
| 目标库位 / 目标状态 | P2 / P4 / P1 | 明确“变到哪里/何种状态” | 仓—区—位级联；状态转移不得绕过质量放行 | 来源目标不清不得发起 |
| 数量与原因 | P7 / P1 | 受控变化依据 | 计划外/报废必须有合法原因与审批 ❓ | 勿用计划外消掉盘点差 |
| 申请/任务/记录状态 | P9 | 门禁 | 相关类型可建预计出/入；完成建事务并清理预期；转移预计入存在分支差异（`GAP-042`） | 完成了但库存没变先查记录与事务 |
...
详情页建议按“作业原因与类型、来源与目标、物料与数量、现场执行、库存影响、异常、系统信息”分组。
