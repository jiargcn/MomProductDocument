...
> 阅读对象：测试、实施、运维（主）；仓库主管、库内作业、库存管理人员（顺带）。

## 业务目的与适用范围

库内作业处理库存仍在企业内部、但地点、状态或数量需要受控变化的场景，例如调拨、库内转移、库存状态转移、计划外出入库和报废。它的目标是让每一次内部变化都有来源、执行过程和结果记录，避免直接改余额造成无法追溯的账实差异。读完本页，应能判断一次库存地点/状态变化该走哪种作业类型、为什么不能用它顶替盘点差异处理。
