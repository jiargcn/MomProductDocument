...
完整语义与对象族边界见[维护与查询参考](生产收料-维护与查询参考.md)。**注意 `GAP-041`：** 本菜单的「生产收料」与[生产管理](../08-生产管理/index.md)的「制品收货」是**两套对象族**，不得混写字段与状态。

| 字段/配置点 | 行为模式 | 在系统中的作用 | 关键行为要点（取值/范围/联动/门禁） | 维护或操作时要警惕什么 |
| --- | --- | --- | --- | --- |
| 生产来源 / 工单 | P2 / P5 | 产出归属 | 任务多由 MES/生产来源生成；本菜单未见与收货同级的申请入口 | 找不到任务时先查 MES 完工，而非制品收货菜单 |
| 产出物料与数量 | P2 / P7 | 应收范围 | 与完工事实一致；实收差异须留原因 | 用错对象族会导致追溯断裂 |
| 收料库位 / 库存状态 | P2 / P4 / P1 | 落点与分流 | 合格/待检/隔离分流；待检不得按合格入库 | 状态错导致误用 |
| 批次 / 包装 | P8 / P3 | 入账粒度 | 见[库存管理精度与唯一粒度](../../02-业务模型/08-库存管理精度与唯一粒度.md) | 漏采无法入账或粒度不足 |
| 任务/记录状态 | P9 | 门禁 | 本链以任务、记录为主；申请是否存在 ❓ | 勿套用制品收货 ATR 结论 |
