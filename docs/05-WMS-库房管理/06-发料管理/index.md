...
> 阅读对象：测试、实施、运维（主）；生产计划、仓库备料/发料、线边人员（顺带）。

## 业务目的与适用范围

发料管理把生产需求转化为仓库可执行的备料、发料和补料工作，使物料在正确时间、以正确数量和追溯维度离开仓库、进入生产现场。它连接生产计划或工单、库存可用量、现场扫码执行和生产用料记录，避免无来源、无记录的随意出库。

相对相邻分组：本页管「仓库往产线发料」；[生产收料](../07-生产收料/index.md)管「产线产出交回仓库」；[生产管理](../08-生产管理/index.md)管「制品收货/完工撤销等生产结果入库」。MES 负责计划与工单执行本身，本页只承接「按需求发料」的仓储侧动作。读完本页，应能判断：某工位缺料是发料没做、库存不足还是补料未触发；少发/超发后系统留下了什么痕迹；以及为何不能用本菜单解释完工入库。

## 如何使用本组文档

| 你的目的 | 建议阅读 |
| --- | --- |
| 理解一笔生产用料怎样走完、库存如何减少 | 本页：准备 → 生产用料如何完成 → 关键判断 → 库存影响 |
| 弄清拣选/扫描/少发超发等约束如何改变现场行为，或据此验证/排障 | 本页主线中的配置说明、「建议验证点」「常见问题与处理」 |
| 具体发起、拣选、补料/退料边界或查字段细节 | [发料管理-维护与查询参考](发料管理-维护与查询参考.md) |
| 对照共享模型 | [申请、任务与记录模型](../../02-业务模型/01-申请任务记录模型.md)、[库存数据挂接模型](../../02-业务模型/02-库存数据挂接模型.md)、[库存管理精度与唯一粒度](../../02-业务模型/08-库存管理精度与唯一粒度.md) |
...
现场能否改量、是否必须扫描批次/包装/库位、少发/超发是否允许，主要由任务所属[业务类型](../../04-DBC-主数据管理/05-策略设置/03-业务类型.md)下发；单据号规则见[单据设置](../../04-DBC-主数据管理/05-策略设置/04-单据设置.md)。变更这些约束前，应在测试环境走通一笔「需求→任务→记录→余额减少」并留回退路径。高低储/自动备料策略启用条件 ❓，未证实前不要培训为必然触发。

!!! example "📝 示例数据占位"
    工单需领用 100 件物料，仓库先发 80 件、后补 20 件。展示备料、任务、发料记录、补料和库存变化。

!!! example "写实示例：给定配置 → 期望行为"
    **给定：** 工单 WO-3001 需物料 A 100 件；发料任务配置为「必须扫描包装」「允许少发」「不允许超发」；线边库位 L-01；现场先实发 80 件。
    **期望：**

    1. 申请/任务可从来源工单带回物料与计划量；未下达或已关闭的工单通常选不到（精确状态集 ❓）。
    2. 未扫描包装或扫描与任务物料不符时不能提交发料。
    3. 提交后：发料记录实发 80；库存事务与余额按 80 减少；剩余需求 20 仍可继续发或走补料，不得用超发一次抹平。
    4. 库存查询应按「发料记录 → 库存事务 → 库存余额」看到扣减，不以任务“已完成”代替余额结果。
    5. 预计出是否随任务生成/释放尚待核验（见下文虚线）；排障时先查记录与事务，不要只盯预计出。
...
- 从有效工单发起发料，核对拣选余额过滤、扫描门禁与实发后余额减少。
- 分别在「允许 / 不允许少发或超发」下验证部分发料、补料触发线索与超发拦截。
- 「库存有数但发不出」：核对库存状态、冻结、批次/包装粒度与任务允许范围。
- 扫描物料或包装不匹配时，确认是否被拦截且不得强行完成。
- 发料完成但生产侧查不到：依次查发料记录 → 库存事务 → 领用地点/生产来源联查。
- 现场反馈“不能改量/必须扫包装”时，先查任务所属业务类型的现场控制，再查权限。
...
| 关联业务 | 应关注什么 |
| --- | --- |
| 生产计划/工单（MES） | 发料是否匹配需求来源、时间和数量；本页不维护 MES 计划本身。 |
| 库存管理 | 拣选前后的可用量、批次/包装、库位和库存状态。 |
| 生产收料 / 生产管理 | 产出交仓、制品入库属另两组菜单，勿与发料混查。 |
| 生产退料 | 线边余料或错料回退应走退料入口，不要用负向改量伪装。 |
...
| 误把完工入库当成发料问题 | 产出入库应查[生产收料](../07-生产收料/index.md)或[生产管理](../08-生产管理/index.md)，本页只解释出库拣选。 |
