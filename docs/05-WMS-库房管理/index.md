...
> 阅读对象：测试、实施、运维（主）；仓库主管、收发货/库内作业、生产物流、盘点、采购/质量协同、现场 PDA 使用人员（顺带）。

## 模块解决什么问题

WMS 把「要收、要发、要搬、要盘」落实成现场可执行的申请/任务/记录，并用**预计 → 事务 → 余额**回答库存「将发生、已发生、现在还剩」。读完本页，应能：分清入库/出库/库内/盘点该进哪组；库存对不上时先查预计、事务还是余额；并知道主数据找 DBC、现场规则以各业务页为准（PDA 不是独立业务模块）。

覆盖：采购收货与上架、发料与生产收料、销售出库、库内移动、盘点，以及库存查询与终端操作。不同业务的状态、权限、数量规则**不能因名称相似互相套用**，须在各分组页确认。

## 如何使用本模块

| 你的目的 | 建议怎么做 |
| --- | --- |
| 理解 WMS 整体定位、一笔仓储业务如何形成库存结果 | 本页：模块解决什么问题 → 一笔仓储业务如何形成库存结果。 |
| 现场执行入库、出库、库内或盘点业务（收发货、生产物流、发运、库内作业、盘点人员） | 按下方“建议学习与操作顺序”，先理解库存预计/事务/余额，再进入对应业务分组的主文档。 |
| 排障/答疑/查询：任务找不到、库存对不上、差异怎样处理 | 本页“常见问题与处理”；库存本身查库存管理，具体业务回对应分组主文档。 |
| 现场用 PDA/线边端执行 | 进终端操作，再回对应 Web 业务页确认规则，不单独定义业务逻辑。 |
| 进入具体分组细读维护/操作方法 | 见下方“业务分组与学习入口”，按建议顺序进入分组。 |
...
这张图是 WMS 的统一理解框架：申请、任务、记录分别表达业务意图、现场工作和实际结果；预计、事务、余额分别用于观察预期、变动和当前结果。采购收货已作为纵向样板验证了“任务形成预计入、记录形成库存事务、事务更新余额”的链路；其它业务是否采用相同对象、何时更新或是否存在反向处理，仍应以对应业务页为准。
...
当前可先从[采购收货](03-采购收货/index.md)和[库存管理](09-库存管理/index.md)理解一条已建立样板的入库与库存追溯路径。其它分组会在第一阶段按上述顺序补齐分组说明、学习导航和页面大纲。
...
| 采购收货样板 | [采购收货](03-采购收货/index.md)、[采购收货-维护与查询参考](03-采购收货/01-采购收货-维护与查询参考.md) |
