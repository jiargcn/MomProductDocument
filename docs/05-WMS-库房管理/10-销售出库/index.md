...
完整选择器、拣选核验与门禁见[维护与查询参考](销售出库-维护与查询参考.md)。相对[采购收货](../03-采购收货/index.md)：对端从供应商变为**客户**；预期从预计入变为**预计出**；执行时强制 **P3 余额核验**。

| 字段/配置点 | 行为模式 | 在系统中的作用 | 关键行为要点（取值/范围/联动/门禁） | 维护或操作时要警惕什么 |
| --- | --- | --- | --- | --- |
| 来源销售订单 / 发货计划 | P2 / P5 | 确定交付范围 | 可选范围受来源状态与业务类型约束（❓） | 选错来源会发错客户或物料 |
| 客户 / 客户月台 | P2 / P5 | 交付对象与装运点 | 客户常回填锁定；月台与客户一致性有校验线索 | 月台与客户不匹配导致装运错位（`GAP-047`） |
| 出库库存状态范围 | P1 / P3 | 限制可拣库存 | 未放行/隔离/冻结通常不可发（❓ 完整集合） | “有数发不出”先查状态 |
| 拣选库位 / 余额 | P3 / P2 | 从哪出库 | 按库存唯一粒度核验可用量，见[库存管理精度与唯一粒度](../../02-业务模型/08-库存管理精度与唯一粒度.md) | 扫错批次/包装 |
| 计划 / 实发数量 | P7 | 控制出库量 | 少发/多发受任务配置 | 缺货假装完成 |
| 申请/任务/记录状态 | P9 | 门禁 | 处理申请生成任务并建预计出；完成清理预计出 | 备货/退货/结算出库另链，勿混用 |
