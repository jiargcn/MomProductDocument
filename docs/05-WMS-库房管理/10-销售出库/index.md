...
> 阅读对象：测试、实施、运维（主）；销售协同、仓库备货/发运、客户交付人员（顺带）。

## 业务目的与适用范围

销售出库把客户交付需求转化为仓库可执行的备货、发运和实际出库结果。它连接销售订单或发货计划、成品可用库存、现场拣货与装运，以及库存扣减和后续客户/外部协同，确保“要发什么、发给谁、从哪里发、实际发了多少”能够完整追溯。读完本页，应能判断一笔发货为何卡在申请/任务某一步、少发或缺货后系统留下了什么处理路径，以及备货、客户退货等相邻入口为何不能共用这条主链。
