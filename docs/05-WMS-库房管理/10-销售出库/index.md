...
支持备货、发货、客户退货和结算出库等成品交付相关的仓储业务。

## 建议学习与操作顺序

| 顺序 | 建议先看什么 | 为什么 |
| --- | --- | --- |
| 1 | 本页的对象关系和边界 | 销售需求如何变为现场出库和可追溯结果。 |
| 2 | 本分组的主业务页或查询页 | 理解一笔典型业务如何开始、执行和结束。 |
| 3 | 对应的库存管理和终端操作页面 | 追溯库存结果，并理解 Web 与现场执行差异。 |

## 关键业务对象与关系

【图示占位：销售来源 → 备货/发货/退货 → 任务/记录 → 库存与交付结果。需以业务对象表达，不使用表名、字段或服务调用；状态和分支待测试验证后补充。】
...
> 本节以 dev 基线 WMS 服务实现为准，用于替代本页上方历史草稿中尚未逐项验证的字段、状态机和自动联动描述。未在本节确认的内容，不应直接作为培训规则或现网承诺。

### 业务对象与主链

标准成品发货使用三层单据：`request_deliver_main/detail`（发货申请）、`job_deliver_main/detail`（发货任务）和 `record_deliver_main/detail`（发货记录）。申请进入处理状态时生成任务；任务按推荐结果拆分后落库，并以任务号为关联键创建预计出。

发货申请可由备货记录生成：服务要求备货记录存在、已关联发货计划且有明细，再带入客户、月台、发货类型、销售订单等信息。该入口只是已证实的生成方式之一，不能据此推定所有销售出库都必须先经过备货或发货计划。

```mermaid
flowchart LR
    A[发货申请] -->|处理| B[按推荐/拆分生成发货任务]
    B --> C[创建预计出]
    C --> D[执行发货任务]
    D --> E[生成发货记录]
    E --> F[出库库存事务]
    F --> G[清理任务预计出]
    E -.受业务规则控制.-> H[接口信息/外部系统]
    I[发货冲销] -.受业务规则控制.-> H
```
...
| 环节 | 已证实的当前行为 | 业务含义 |
|---|---|---|
| 申请转任务 | 申请处于处理状态时调用任务生成；任务可按推荐出的库存明细拆分。 | 同一申请可形成多张任务，培训中不能将申请号等同于唯一执行单。 |
| 业务类型快照 | 任务生成时读取 `Deliver` 业务类型，并带入允许改库位、数量、批次、包装、大小数、重复扫描等运行参数。 | 现场可改范围取决于业务类型配置；具体开关的 UI、权限和终端表现仍需单独核验。 |
| 预计出 | 每条任务明细以来源库位、来源批次、货主、数量、来源库存状态等信息创建预计出，并关联任务号。 | 预计出是任务执行前的库存占用/预期信息，不等同于已经扣减库存。 |
| 发货类型 | 非 `CUST` 发货类型会写入业务类型配置的在途库位；申请由备货记录生成时，`DIRECT_SHIPMENT` 不填目标库位。 | 客户交付、在途/寄售等场景不能直接套用同一库位流转结论。 |
| 翻包前置 | 非 `DIRECT_SHIPMENT` 的任务生成后会触发翻包申请创建。 | 该动作的执行状态、失败回滚及与终端扫描的关系尚未闭合。 |

### 执行、库存与记录

任务执行会生成正常状态的发货记录，并以实际处理数量写入记录明细。执行时按来源库位的库存管理精度处理：数量管理、批次管理和包装管理走不同的包装/批次参数；唯一码管理当前直接抛出“不支持”异常。系统按物料、包装（或父包装）、批次、来源库位、库存状态查询余额；扫描子包装无余额而带父包装时会尝试以父包装余额继续，否则拒绝执行。

当业务类型配置了出库事务类型时，服务根据事务类型取得库存动作，创建出库库存事务；执行完成后删除任务号对应的预计出。记录还会带出来源/目标库位、库存状态、包装、批次、货主和成本等执行快照。因而培训应将“任务执行成功”理解为库存事实发生的触发点，而不是申请创建或预计出创建时点。

### 接口与冲销边界

发货冲销会生成独立的冲销记录主子表，并将数量取反。仅当“记录创建后规则”启用 `toExternalSystemsTransferInterface`、指定接口类型且接口类型包含 WMS—ERP 库存移转接口时，才会通过 Redis 消息创建外部接口信息；冲销接口信息的名称为“成品发货冲销”。这证明接口推送由业务规则控制，不能把页面上的“已发货”状态直接解释为外部系统已经成功入账。

### 建议列表、详情与跳转样板

| 页面 | 列表优先字段 | 详情分组建议 | 快速跳转 |
|---|---|---|---|
| 发货申请 | 申请号、状态、客户、发货类型、销售订单、计划/备货来源、需求时间、创建时间 | 基本信息；客户与计划；物料明细；处理与审批；来源追溯 | 发货计划、备货记录、发货任务、销售订单、客户 |
| 发货任务 | 任务号、状态、申请号、客户、发货类型、来源库位、预计出数量、需求时间 | 执行要求；来源库存与预计出；包装/批次；执行记录；接口 | 发货申请、预计出、库存余额、发货记录、翻包申请 |
| 发货记录 | 记录号、执行状态、任务号、客户、发货类型、实际数量、执行人、执行时间、接口状态 | 执行快照；库存事务；包装/批次；冲销；外部接口与趋势 | 发货任务、库存事务、库存余额、接口信息、冲销记录 |

### 待补齐的问题

- 备货、发货计划、客户退货、寄售/在途、结算出库均有独立服务域；其单据关系、状态机和库存链路需要逐项取证，不能以标准发货链替代。
- 申请/任务/记录的创建、编辑、撤销、删除、导入模板、PDA/线边端入口和后端 RBAC 尚未完成端到端核验。
- 预计出创建已确认；取消或部分执行时预计出如何恢复/扣减、翻包失败如何补偿、接口重试与幂等规则仍待验证。
- 本页上方“FIFO 自动分配、库存预警、物流承运商同步、退货自动建质检单”等历史描述尚未在本轮源码取证中确认，保留为待核验信息而非当前规则。
