...
本页讲清采购退货为何发生、怎样从问题来料走到退货结果，以及应怎样查询。具体字段、状态、按钮、PDA 和外部回写细节需要在后续采购退货-维护与查询参考与测试环境中补齐。
...
完整选择器范围、可退范围与门禁见[维护与查询参考](采购退货-维护与查询参考.md)。与[采购收货](../03-采购收货/index.md)同属申请—任务—记录链，但**来源与数量方向相反**；下列只列主线差异项。

| 字段/配置点 | 行为模式 | 在系统中的作用 | 关键行为要点（取值/范围/联动/门禁） | 维护或操作时要警惕什么 |
| --- | --- | --- | --- | --- |
| 来源收货记录 / 可退范围 | P2 / P5 | 确定退哪一次到货、退哪些明细 | 通常从已完成收货结果发起；可选范围受已收、已退、已上架/检验等消耗约束（❓ 精确过滤待确认） | 选错来源会退错供应商或批次 |
| 供应商 | P2 / P5 | 退回对象 | 多由来源收货回填并锁定 | 与实物交接方不一致导致对账失败 |
| 可退/实退数量 | P7 | 控制退货量 | 不得超过可退范围；超退应被拦截（❓ 默认策略待确认） | 强改数量造成账实与供应商协同错位 |
| 退货原因 / 质量结论 | P1 | 分流质量退、超收退、业务退等 | PDA/Web 退货需保留原因；质量触发场景应可联查检验结论 | 无原因无法闭环异常 |
| 库位 / 批次 / 包装 | P2 / P3 / P8 | 从哪一笔余额拣出退货 | 按库存唯一粒度定位，见[库存管理精度与唯一粒度](../../02-业务模型/08-库存管理精度与唯一粒度.md) | 漏采或错位导致扣错库存 |
| 申请/任务/记录/撤销状态 | P9 | 动作门禁 | 另有撤销记录入口；状态码 ❓ | 勿把撤销当删除历史 |
