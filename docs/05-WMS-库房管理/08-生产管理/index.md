...
WMS 生产管理处理生产结果如何进入、回退或转换仓储库存，例如制品收货、完工撤销、隔离收货、制品上架，以及拆解、返修等相邻场景。它连接生产完成事实、仓库接收、库存结果，以及可能触发的上架、质量或外部回写。读完本页，应能判断一笔生产产出该走制品收货还是隔离收货、收货后是否需要继续上架或送检，以及为何不能把它和`07-生产收料`当成同一套对象。
