...
相对相邻分组：[发料管理](../06-发料管理/index.md)管「仓库往产线发料」；[生产收料](../07-生产收料/index.md)管「产线产出交仓」的另一套任务/记录对象；本页以**制品收货**为主链做生产结果入库与回退。MES 负责计划与工单执行，本页不维护 MES 菜单，只处理仓储侧接收、上架与撤销。读完本页，应能判断：一笔产出该走制品收货还是隔离收货、收货后是否需要继续上架或送检，以及为何不能把它和`07-生产收料`当成同一套对象（`GAP-041`）。

本页是 WMS 侧生产结果处理说明，不是 MES 生产计划/执行模块。通用申请、任务和记录概念见[申请、任务与记录模型](../../02-业务模型/01-申请任务记录模型.md)。

## 如何使用本组文档

| 你的目的 | 建议阅读 |
| --- | --- |
| 理解生产结果如何变成可追溯库存 | 本页：准备 → 一笔制品收货 → 对象边界 → 库存影响 |
| 弄清业务类型/开关如何改变落点与后续，或据此验证/排障 | 本页主线中的配置说明、「建议验证点」「常见问题与处理」 |
| 正在执行制品收货、上架、撤销或查询字段细节 | [生产管理-维护与查询参考](生产管理-维护与查询参考.md) |
| 若处理的是菜单「生产收料」或发料 | 转到[生产收料](../07-生产收料/index.md) / [发料管理](../06-发料管理/index.md) |
...
申请表达“哪些生产结果需要入库”，任务组织仓库执行，记录保存实际接收结果。申请可形成预计入；任务执行后形成库存事务、更新余额并清理预计入。回冲、外部接口、自动上架、质量检验等后续动作受单据开关和规则控制，不能写成每笔收货的固定必经步骤（`GAP-069`）。

业务类型区分制品收货与隔离收货等入口，见[业务类型](../../04-DBC-主数据管理/05-策略设置/03-业务类型.md)；单据号规则见[单据设置](../../04-DBC-主数据管理/05-策略设置/04-单据设置.md)；条件后续还可能受[单据开关](../../04-DBC-主数据管理/05-策略设置/05-单据开关.md)影响。变更前应在测试环境走通一笔「申请→任务→记录→余额」并核对是否误触上架/质检。

!!! example "📝 示例数据占位"
    工单完工 100 件，仓库实收 100 件并形成库存；其中需上架或送检的分支单独标注为条件动作。

!!! example "写实示例：给定配置 → 期望行为"
    **给定：** 生产完成事实 PF-5001，物料 C 计划入库 100 件；业务类型为「制品收货」（合格）；任务要求扫描库位；自动上架开关关闭、质检开关关闭。
    **期望：**

    1. 制品收货申请可带入来源与明细；隔离产出应走隔离收货入口，勿用合格类型强行收。
    2. 处理生成任务后可查到预计入库；执行收货后预计入应清理，并形成库存事务与余额。
    3. 未扫描/校验库位时不能提交；提交后记录实收 100，余额按合格状态落位。
    4. 因开关关闭，不应默认生成上架任务或质检申请；需要时再手工发起（`GAP-069`）。
    5. 同一产出不应在[生产收料](../07-生产收料/index.md)菜单用另一套对象重复入账（`GAP-041`）。
...
- 分别用制品收货 / 隔离收货入口走同一产出，核对库存状态落点差异。
- 开关开/关时验证是否自动生成上架或质检（`GAP-069`）；关闭时不应被培训为“必经”。
- 申请后查预计入，执行后查事务与余额，并确认预计入已清理。
- 完工撤销：已有上架/接口后续时，评估冲抵范围后再执行（闭环细节 ❓）。
- 确认生产收料菜单任务与制品收货任务不可互换查询（`GAP-041`）。
- 发料缺料问题应查[发料管理](../06-发料管理/index.md)，本页只管生产结果入库侧。
