...
> 阅读对象：生产协同人员、仓库收货/上架人员、质量协同人员及需要追溯生产结果入库的业务人员。
...
完整语义见[维护与查询参考](生产管理-维护与查询参考.md)。本分组主链是**制品收货**；与[生产收料](../07-生产收料/index.md)的对象族边界见 `GAP-041`。

| 字段/配置点 | 行为模式 | 在系统中的作用 | 关键行为要点（取值/范围/联动/门禁） | 维护或操作时要警惕什么 |
| --- | --- | --- | --- | --- |
| 业务类型（制品/隔离等） | P1 / P12 | 区分合格收货与隔离收货等入口 | 隔离收货复用制品收货对象、以业务类型区分 | 进错入口导致状态落错 |
| 生产来源与完成事实 | P2 / P5 | 入库范围 | 申请选择或带入；须具备入库条件 | 与生产收料菜单任务不可混查 |
| 物料 / 数量 / 单位 | P2 / P7 | 入账对象 | 与完成事实一致 | 差异须留原因 |
| 库位 / 库存状态 | P2 / P4 / P1 | 落点与可用范围 | 隔离状态走隔离入口；仓区位级联 | 合格/隔离混用 |
| 批次 / 包装 / 托盘 | P8 / P3 | 入账粒度 | 见[库存管理精度与唯一粒度](../../02-业务模型/08-库存管理精度与唯一粒度.md) | 漏采 |
| 条件后续（上架/质检/接口） | P12 | 收货后是否自动继续 | **受开关控制**，非每笔必发（`GAP-069`） | 勿培训为固定必经步骤 |
| 申请/任务/记录状态 | P9 | 门禁 | 申请可建预计入；任务执行建事务并清理预计入 | 完工撤销是独立链 |
...
详情建议分组：生产来源、物料与数量、执行与状态、库存影响、条件后续、系统信息。
