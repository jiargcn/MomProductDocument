...
库存管理回答三个不同的问题：已经安排但尚未完成的入库会带来什么预期、某一次实际业务做了什么库存变动、现在某物料在何处还剩多少。为避免把“待完成”“一次变化”和“当前结果”混为一谈，系统分别使用预计入库、库存事务和库存余额三个对象承载。读完本页，应能判断“库存不足”“账实不一致”一类问题该先查哪一张表，而不是把三者当成同一份结果反复核对。
