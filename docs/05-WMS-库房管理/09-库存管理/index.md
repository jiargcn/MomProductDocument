...
| --- | --- | --- | --- | --- |
| 预计入库存 | 查询、详情、导出。 | 前端新增、导入、编辑、删除按钮当前被注释；虽然存在表单/API Hook，不应直接写成日常人工维护能力。 | 预计入应主要由任务创建或释放，不建议作为普通手工维护页描述。 | 已取证，待测试确认菜单按钮。 |
| 库存事务 | 查询、详情、导出。 | 前端新增、导入、编辑、删除按钮当前被注释；事务通常由业务记录过账生成。 | 库存事务是追溯流水，不应作为普通人工新增/编辑对象。 | 已取证，待服务入口专项确认。 |
| 库存余额 | 查询、详情、导出、包装明细、SN 明细、创建包装。 | 前端新增、导入、编辑、删除按钮当前被注释；导入组件存在但日常入口未展示。 | 库存余额是当前库存状态载体，日常应由事务更新；人工导入更适合定位为初始化/迁移或专项调整。 | 已取证，待导入服务和权限复核。 |
