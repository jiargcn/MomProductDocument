...
| --- | --- | --- | --- |
| 预计入库存 | 详情（任务号可点入）、导出、刷新/筛选/列设置；**批量删除按钮当前未被注释**。 | 新增、导入、逐行编辑/删除按钮均已注释，仍保留 `BasicForm`、`ImportForm` 和相应 API 调用。 | 当前页面并非完全只读：批量删除代码路径可达，且调用 `deleteExpectinIds`。是否符合预计入释放/撤销规则、是否需要权限和状态校验，必须专项核验。 |
...
| --- | --- | --- | --- | --- |
| 预计入库存 | 查询、详情、导出、刷新/筛选/列设置、**批量删除**。 | 新增、导入、逐行编辑/删除按钮当前被注释；但页头批量删除按钮未注释，能调用 `deleteExpectinIds`。 | 预计入仍应主要由任务创建或释放；批量删除的业务前置、权限和状态校验尚未确认，不能当作常规维护建议。 | 已取证，已登记差距。 |
