...
【图示占位：业务任务/记录 → 预计或事务 → 库存余额与追溯。需以业务对象表达，不使用表名、字段或服务调用；状态和分支待测试验证后补充。】
...
这一定义是本轮文档的业务口径。当前 DDL 尚未看到对应的复合唯一约束，属于技术改进项而不是文档阻塞项；详见产品差距总账。
...
| 服务行为 | 已证实实现 | 文档结论 |
| --- | --- | --- |
| 库存事务创建 | `TransactionServiceImpl.createTransaction` 在每条输入上补全库位、管理精度、有效期和事务号；空批次会写为 `0`。 | 业务调用方不能把这些字段是否自动补齐完全交给页面说明；不同调用方式仍需按服务取证。 |
| 数量方向 | 当库存动作为 `OUT` 时，服务将输入数量改为负数后写入事务与余额更新请求。 | 事务数量的正负承载入/出方向；具体动作枚举和值域仍需按事务类型配置核验。 |
| 余额先于流水更新 | 服务先把余额变更请求合并，调用 `balanceMapper.updateBalanceQtyBatch(...)`，再合并并批量写入库存事务。 | “业务记录 → 库存事务 → 库存余额”是业务追溯链；当前服务内部的持久化顺序为“余额更新后写事务”，不能把两者的物理写入顺序误作业务语义。 |
| 物料主数据补全 | 服务根据物料代码查询物料主数据，填入余额请求中的物料类型、名称、描述；查不到物料时返回 `0`。 | 库存过账依赖物料主数据可查询，物料禁用/删除等跨模块规则仍需专项验证。 |
| 特殊订单行 | `poLineType = M` 时不进入余额更新请求构造，但事务仍会进入写入集合。 | 该分支的业务语义和是否会造成“有事务无余额变化”需由业务/测试验证，当前不做推测。 |
...
| 库存事务 | 当前余额详情页签实际按 `item_code`、`package_number`、`batch_id` 查询事务。 | 从余额回看相关事务。 | 已取证；该实现未带入托盘、库存状态、库位等完整余额粒度，可能混入同物料/包装/批次下其它余额行的事务，已登记差距。 |
...
| 结论 | 证据 | 文档使用方式 |
| --- | --- | --- |
| 前端导出入口配置了页面权限标识。 | `expectin/index.vue` 使用 `wms:expectin:export`，`transaction/index.vue` 使用 `wms:transaction:export`，`balance/index.vue` 使用 `wms:balance:export`。 | 可作为“按钮可见性”的候选证据，仍需角色实测。 |
| 本轮未证实上述权限在后端接口层强制执行。 | 三个 Controller 的导出 `@PreAuthorize` 注解均处于注释状态；库存事务创建、预计入删除等关键接口也存在注释的权限注解。 | 不能把前端 `hasPermi` 直接写成接口安全边界；具体授权矩阵需继续取证并登记为产品差距。 |
