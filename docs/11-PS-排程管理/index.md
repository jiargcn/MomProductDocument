...
> 阅读对象：测试、实施、运维（主）；计划员、生产主管（顺带）。
> **侧栏标注「规划中」。** 资料状态薄弱：本模块当前只能写清产品规划意图与 MES/DBC 边界；不能把旧稿字段表、算法指标与未经确认的 ER 当作已落地事实。已落地的下发/开工请读 [MES 计划管理](../06-MES-生产管理/03-计划管理/index.md)。

## 业务目的与适用范围

排程管理（Production Scheduling，PS）在产品规划中，承接「需求/订单 → 产能约束下的可执行计划 → 下发生产」之间的排程层：维护约束主数据、录入待排订单、执行排程、对比版本、查询结果，并以计划视图支撑发布决策。

读完本页，应能：说清 PS 与 MES 计划管理不是同一层；判断现场执行问题该去 MES 还是仍属规划中的 PS；在取证完成前不把旧稿字段当培训事实。

## 如何使用本模块

| 你的目的 | 建议怎么做 |
| --- | --- |
| 了解排程产品意图与边界 | 本页：业务目的 → 与 MES 分层 → 当前可确认状态 |
| 做已落地的订单下发/开工 | 去 [MES 计划管理](../06-MES-生产管理/03-计划管理/index.md)，不要在本模块找执行细则 |
| 查工艺/产线主数据 | [DBC 工艺路线](../04-DBC-主数据管理/08-工艺建模/02-工艺路线.md)、[生产线](../04-DBC-主数据管理/04-工厂建模/07-生产线管理.md) |
| 看各分组规划意图 | 下方业务分组表进入对应页（均只写意图与缺口） |
| 期待字段级排程配置 | 暂不可用——见「当前可确认状态」与 `GAP-017` |
...
!!! example "读完自检"
    **给定：** 现场要拆工单并下发开工。
    **期望判断：** 去 MES 计划管理，而不是在本模块找按钮或字段表。若有人拿旧 PS 字段表做培训，应先对照本页「当前可确认状态」驳回。

## 当前可确认状态（盘点结论）

| 检查项 | 结论 |
| --- | --- |
| `docs/11-PS-排程管理/` 目录结构 | **有**：模块首页 + 6 个业务分组页（与 `mkdocs.yml` 一致） |
...
**因此本轮只能做到：** 保留导航与业务分组骨架，用业务化文风写清意图与边界，并显式标注未证实项；**暂不展开**字段级维护细节与对象级对照。

取证任务见内部清单：内部资料（不参与站点发布）。

### 建议验证点（能力边界）

- 测试环境菜单/路由：确认是否仍无独立 PS/APS 入口（与盘点结论对照）。
- 需要下发/开工时：能在 MES 计划管理走通，且不依赖本模块页面。
- 抽查旧稿字段名/利用率公式：不得出现在现行培训材料中。
...
| API 参考 | 排程相关接口登记入口 | 见 [API 参考](../14-API参考/index.md)；PS **无**伪路径 |
