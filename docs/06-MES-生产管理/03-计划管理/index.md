...
| 协同方 | 本页负责 | 不在本页展开 |
| --- | --- | --- |
| 工艺管理 | 引用路线编码/版本；下发时落运行快照 | 路线图形与节点维护 |
| 终端操作 | 提供可开工的运行工单 | 扫码、工位作业、装箱细节 |
| WMS | 触发投料/完工入库的业务前提 | 库存事务与余额规则 |
| QMS | 过程检验可挂在执行节点 | 检验方案与判定 |
...
- 生产订单 `status`/`systemStatus` 全量字典文案待测试环境核对；
- 待点检是否强制、点检通过动作入口待联调；
- 已退料与发料回冲的完整闭环待与 WMS 发料页对齐；
- 菜单中另有「工单管理/工单执行/生产计划」等历史或旁路入口，是否与主链并存以环境菜单为准，不在本刀展开细则。
