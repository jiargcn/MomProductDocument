...
| 你的目的 | 建议阅读 |
| --- | --- |
| 理解订单如何变成可执行工单、状态如何流转 | 本页：准备 → 从订单到执行 → 关键判断 → 边界 |
| 弄清下发/快照/并行组/任务模式如何改变现场，或据此验证 | 本页「写实示例」「建议验证点」「常见问题」 |
| 具体建单、拆分、下发、开工或查字段/状态动作 | [计划管理-维护与查询参考](计划管理-维护与查询参考.md) |
| 核对路线版本与运行快照 | [工艺管理](../02-工艺管理/index.md) |
| 查投料发料或完工入库 | [WMS 发料](../../05-WMS-库房管理/06-发料管理/index.md) / [生产收料](../../05-WMS-库房管理/07-生产收料/index.md) / [生产管理](../../05-WMS-库房管理/08-生产管理/index.md) |
...
下发时写入运行工单与工艺快照；线边只消费运行层。完工后库存落地不在本页——联查 WMS 生产收料/制品收货；投料侧联查 WMS 发料。

!!! example "📝 示例数据占位"
    ERP 单 EO-01 → 生产订单 PO-M01 → 工单 WO-4001/WO-4002（拆分）；下发后运行快照 RT-01 v2。展示转换、拆分与快照版本。

!!! example "写实示例：给定配置 → 期望行为"
    **给定：** ERP 订单 EO-01 未转换；物料 P-100、数量 100；拆成 WO-4001/WO-4002 各 50；均绑定路线 RT-01 v2、任务模式=单件、产线 L1；WO-4001 下发并开工后，计划员把计划队列序从 1 改为 5。
    **期望：**

    1. 验证转换→执行转换后，生产订单可追溯到来源 EO-01；已转换勿重复转换。
    2. 下发成功：出现运行工单，并带 RT-01 v2 快照；缺路线或产线时应失败并提示。
    3. 开工后线边可按单件模式领作业/报工；任务模式若误选批量，报工入口与追溯粒度会不符。
    4. 已下发后改计划队列序：可能不回写运行表——现场顺序仍以运行层为准，勿以为改序即改现场。
    5. 工单已完工但仓库无账：先查完工记录，再查 [WMS 生产收料](../../05-WMS-库房管理/07-生产收料/index.md) / [生产管理](../../05-WMS-库房管理/08-生产管理/index.md)，不要在 MES 改库存。

### 建议验证点

- ERP：验证转换→执行转换→生产订单可查到来源。
- 拆分 ≥2 张工单，数量之和可核对。
- 下发后改主数据路线：在制仍走快照；新单才吃新版本。
- 单工单：下发→开工→挂起→恢复→完工（或报工自动完工）可走通。
- 并行组：创建→统一下发→整组开工；半组单独下发应能被规则拦住或培训禁止。
- 已下发改计划队列序：行为符合“不回写运行表”口径。
- 完工有数、库存无数：按完工记录 → WMS 收货/入库联查。
