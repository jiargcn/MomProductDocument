...
| --- | --- | --- | --- | --- |
| 路线编码 + 版本 | P6 / P10 | 工艺身份与改版边界 | 改工艺优先升版；在用版本被计划/快照引用后勿覆盖 | 覆盖在用版会导致在制与追溯错位 |
| 物料 / BOM / BOM 版本 | P2 / P4 | 路线适用对象与用料版本 | 先选物料再绑 BOM；产品关联回显依赖此组 | 选错物料会把错误路线挂到计划 |
| 路线状态 | P1 / P9 | 能否被计划引用 | 文案与启停以页面字典为准（`MES-ROUTE`） | 勿臆造 DRAFT/ACTIVE 当培训事实 |
| 工序节点 / 前后序 | P2 / P4 | 执行路径 | 保存图形后须落明细；工序来自主数据 | 只改图未保存 → 现场无节点 |
| 最小合格转序数量 | P7 / P9 | 批量转序门禁 | 合格累计达阈值才可转下序 | 过严堵线、过松提前流转 |
| 批量转序策略 | P1 / P12 | 按工单累计放行或按任务立即转 | 空配置默认工单累计达标 | 策略与报工粒度不匹配会卡转序 |
| 并行 / 并行上限 | P1 / P9 | 同工序多工位 | 并行开启时上限须合法，否则保存失败 | 非法并行配置无法保存 |
| 作业分派模式 | P1 / P12 | 抢单或派单 | 空=未配置；与工位配置一起验收 | 选错则线边领取方式不符 |
| 工序—工位 / 主工位 | P2 / P11 | 现场可执行工位 | 多工位须明确主工位 | 工位对不上 → 抢单/派单失败 |
| 运行快照 | P5 / P9 | 执行侧冻结工艺 | **工单下发时**落库；在制读快照非最新主数据 | 改主数据不等于改在制 |
