...
> 阅读对象：测试、实施、运维（主）；工艺/线边实施、生产主管等现场角色（顺带）；维护步骤见[基础建模-维护与查询参考](基础建模-维护与查询参考.md)。

## 业务目的与适用范围

基础建模维护线边执行所需的配置底座：线边客户端、SOP 与挂接、工位技能要求、开工点检模板、线边库/料口映射，以及生产 BOM（工序用料）。工厂/产线/工位主数据以 DBC 为准；人员技能等级主数据当前菜单多在 DBC；本页写 MES 侧已证实配置。读完本页，应能判断某产线开不了产、终端登不上或缺 SOP，究竟缺了哪一项线边配置。
...
| 你的目的 | 建议阅读 |
| --- | --- |
| 理解开产前要配齐哪些线边底座 | 本页：准备 → 一次建模 → 关键判断 → 边界 |
| 弄清绑错工位/缺 SOP/线边库未映射会怎样，或据此验证 | 本页「写实示例」「建议验证点」「常见问题」 |
| 具体配客户端、SOP、点检、线边库或查字段 | [基础建模-维护与查询参考](基础建模-维护与查询参考.md) |
| 看路线与工单如何消费这些配置 | [工艺管理](../02-工艺管理/index.md)、[计划管理](../03-计划管理/index.md) |
| 看线边怎么操作 | [终端操作](../06-终端操作/index.md) |
...
底座配齐后，计划才能下发/开工，终端才能领到正确工位作业；线边库映射正确后，缺料才应去查 [WMS 发料](../../05-WMS-库房管理/06-发料管理/index.md)，而不是在 MES 里改库存。

!!! example "📝 示例数据占位"
    产线 L1、工位 W1/W2、客户端 C-01；物料 A/B 各挂一份工序 SOP；线边库 LB-01 ↔ WMS 仓 WH-LB。展示绑定、挂接与映射关系。

!!! example "写实示例：给定配置 → 期望行为"
    **给定：** 客户端 C-01 绑定产线 L1、工位集合 {W1}；工序 OP10 对物料 A 挂 SOP-v2；线边库 LB-01 映射 WMS 仓 WH-LB；工位 W1 要求技能 SK-01≥2 级。
    **期望：**

    1. 用绑定用户登录 C-01 后，只能看到 W1 相关作业池；绑到 W2 的作业不应出现。
    2. 在 OP10 做物料 A 时终端打开 SOP-v2；物料 B 未挂接则不应误显 A 的指导书。
    3. 人员技能低于要求时，上岗/领作业应被拦截（若项目启用技能校验）；勿绕过配置强行开干。
    4. 线边无料时：先核对 LB-01↔WH-LB 映射，再联查 WMS 发料记录与余额；映射错误会导致“发了但线边看不到”。

### 建议验证点

- 新客户端绑定产线+工位后，终端可登录并只见对应工位作业。
- 同工序两物料挂不同 SOP，终端展示正确版本。
- 工位技能过严/过松各验一单（启用技能校验时）。
- 开工点检模板覆盖样例路线后，工单可走完待点检→待开工。
- 故意错映射线边库 WMS 编码，确认发料/线边库存对不上；改回后可联查。
...
| --- | --- | --- | --- |
| 线边客户端↔工位/工序 | 决定能领哪些作业 | 绑定须匹配产线 | 绑错领不到活 |
| SOP 挂接层（工序/工位） | 现场指导书来源 | 明确以哪层展示为准 | 版本不一致 |
| 工位技能要求 | 上岗拦截 | 引用技能/等级 | 过严/过松 |
| 开工点检模板 | 开工前检查 | 与计划「待点检」联动 ❓ | 与 EAM 点检混淆 |
| 线边库 WMS 映射 | 库存协同键 | 须有效 WMS 仓/库编码 | 对不上发料 |
| 生产 BOM | 工序用料 | 与 DBC BOM 孰为准 ❓（`MES-BASE`） | 用料脱节 |
...
| WMS | 线边库编码映射与协同前提；缺料联查发料入口 | 库存事务与余额；发料/生产收料动作本身见 [发料](../../05-WMS-库房管理/06-发料管理/index.md) / [生产收料](../../05-WMS-库房管理/07-生产收料/index.md) |
