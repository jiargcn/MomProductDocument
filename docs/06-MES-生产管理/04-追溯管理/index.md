...
| --- | --- | --- | --- | --- |
| 查询起点（物料码 / SN / 批次） | P2 / P8 | 决定正向入口与粒度 | 三者不可混用期望同一结果集 | 粒度错 → 空结果或过大结果 |
| 逆向条件（追溯码 / 产品 SN / 工单等） | P2 / P8 | 定位追溯任务 | 尽量带唯一码或工单号 | 条件过宽难排查 |
| 物料消耗树 | P5 / P8 | 展示消耗链路 | 节点来自工位消耗日志；↔WMS 出库单号 ❓（`MES-TRACE`） | 勿把树节点当可编辑库存 |
| 人员 / 工序 / 设备参数页签 | P5 / P14 | 联查过程事实 | 分别来自报工日志、工位作业、设备参数追溯 | 无采集则页签空 |
| 追溯码规则（按物料） | P1 / P12 | 约束赋码与解析 | 规则须覆盖该物料 | 规则缺失 → 码无法识别 |
| 回收件展示 | P1 | 物料属性语义 | 若页面误显「标准件」，按「回收件」理解（`GAP-007`） | 培训勿跟错误标签 |
