...
| WMS | 投料/完工引起的库存结果查询入口提示 | PDA 收发货任务页 |
| QMS | 报工 NG/不良可触发质量协同线索 | 检验方案与评审 |
...
- 线边前端路由 `/mesprocess` 下各子页与按钮显隐待 UI 实测补截图；
- 通用离线缓存、断网续传未在本刀证实；
- 生产报工 Web 页（`reportForWork`）与工位作业报工 API 的职责切分待对照前端；
- NG 报工到 QMS 的精确单据映射待质量页闭合。
