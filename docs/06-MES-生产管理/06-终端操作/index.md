...
> 阅读对象：测试、实施、运维（主）；线边班组长、操作工培训讲师等现场角色（顺带）；操作步骤见[终端操作-维护与查询参考](终端操作-维护与查询参考.md)。

## 业务目的与适用范围

终端操作说明 **MES 线边客户端** 上如何围绕已下发/已开工的运行工单完成：人员上下工、领取工位作业、开始作业、单件/批量报工、关注转序门闸，以及与开工点检、并行组布局相关的现场差异。读完本页，应能判断操作工领不到活、报工失败或转不了序，该查客户端绑定、任务模式还是工艺门槛。
...
| 你的目的 | 建议阅读 |
| --- | --- |
| 理解线边一天怎么干活（领活→报工→转序） | 本页：准备 → 现场主路径 → 关键判断 → 边界 |
| 弄清任务模式/分派/门闸如何改变现场，或据此验证/排障 | 本页「写实示例」「建议验证点」「常见问题」 |
| 培训操作工或逐步排查领不到活 | [终端操作-维护与查询参考](终端操作-维护与查询参考.md) |
| 改终端绑定或 SOP | [基础建模](../01-基础建模/index.md) |
| 查报工/上下工历史 | [报表统计](../05-报表统计/index.md) |
| 线边缺料或完工入库 | [WMS 发料](../../05-WMS-库房管理/06-发料管理/index.md) / [生产收料](../../05-WMS-库房管理/07-生产收料/index.md)（非本页 PDA） |
...
本页只写 **MES 线边客户端**。WMS PDA 上的发料/收料任务属仓储终端，勿混训。

!!! example "📝 示例数据占位"
    工单 WO-7001（单件、抢单）、OP10 门闸最小合格=5；客户端 C-01 绑定 W1。展示领取→报工→门闸放行。

!!! example "写实示例：给定配置 → 期望行为"
    **给定：** 运行工单 WO-7001 已开工、任务模式=单件、分派=抢单；客户端 C-01 仅绑定工位 W1；工艺 OP10 最小合格转序=5；现场先报 OK×4。
    **期望：**

    1. 登录 C-01 并上工后，可抢到 W1 待分派作业并开始；绑在 W2 的作业不应出现在本机。
    2. 须走单件报工入口；误用批量入口应失败或不可用。
    3. OK 累计未达 5 时转序门闸不放行；第 5 笔 OK 后才可转下序（策略为工单累计时）。
    4. 线边无料：查线边库存 + [WMS 发料](../../05-WMS-库房管理/06-发料管理/index.md)，核对基础建模线边库映射；不要在终端改余额。
    5. 报工 NG → QMS 自动建单 **未证实**（`GAP-071`）；培训只保留不良协同线索，不写成固定建单。
...
- 抢单：待分派可认领并单件报工成功。
- 派单：指定工位出现待执行并可报工。
- 批量门闸：累计达到前后转序行为符合工艺配置。
- 并行组：扫码命中正确工单 Tab。
- 确认培训未把 WMS PDA 当作 MES 终端。
...
| 离线细则未证实却写成已支持 | 不宣称通用离线能力；待前端取证后再写。 |
