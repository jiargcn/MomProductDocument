...
> 基线状态：已完成 `dev` 后端首轮取证。本页已有的字段、接口和状态机多为待确认材料；以下对象范围以当前控制器为依据。

## 已核验的模块职责

ANDON 当前后端具备问题记录、问题响应链配置、问题响应/处理记录、整改与整改跟踪、问题文档记录、停机项目、故障、消息配置/发送和 OEE 确认等对象。它承担异常从记录到响应、处置、跟踪和分析的业务闭环。

| 能力域 | 当前已核验对象 | 文档边界 |
| --- | --- | --- |
| 异常事实 | 问题记录、故障、停机项目。 | 异常来源可能来自人工、MES、EAM 或数采；实际来源和触发接口待核验。 |
| 响应与处置 | 响应链配置、问题处理记录、整改、整改跟踪。 | 响应人匹配、超时升级、关闭/复位状态待核验。 |
| 证据与通知 | 问题文档记录、消息配置、消息发送。 | 通知渠道和平台消息服务的复用关系待核验。 |
| 分析 | OEE 确认、停机项目等分析对象。 | 与 MES 生产时长、EAM 设备指标的统计口径待核验。 |

## 关键边界

1. ANDON 记录“异常事件与处置闭环”，不替代 EAM 的设备资产/维修工单，也不替代 MES 的生产执行记录。
2. 设备信号或人工上报如何转化为异常记录、是否自动创建维修或质量处理任务，必须在跨模块追溯页中按关联键核验。
3. ANDON 的消息配置属于业务事件配置；消息通道、投递、失败重试和审计应归平台能力统一治理。
