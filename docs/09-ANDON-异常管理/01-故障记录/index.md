...
> 阅读对象：测试、实施、运维（主）；班组长、安灯管理员、设备/质量协同（顺带）。操作见[故障记录-维护与查询参考](故障记录-维护与查询参考.md)。

## 业务目的与适用范围

产线一声呼叫响起，谁去处理、多久该有人到场——这些痕迹沉淀在故障记录里。它记录产线异常的**发生事实**与过程痕迹：谁在何时对哪台设备/模具/工单发起呼叫，问题分类与来源是什么，处理措施与附件是什么。

与 EAM 的边界必须分清：本页是**呼叫与到岗事实**；[EAM 报修维修](../../08-EAM-设备管理/02-设备管理/index.md)是**维修工单状态机**。呼叫结束 ≠ 维修完成；「维修单状态」字段只是线索，**不自动等于**已开维修工单。也不替代 QMS 检验记录。来源码已预留安东异常、设备巡检、质量检验。旧稿虚构英文等级状态机与 REST 示例废弃。

读完本页，应能跟完一笔呼叫→到岗→结束，并判断何时另开 EAM 报修。

## 如何使用本组文档

| 你的目的 | 建议阅读 |
| --- | --- |
| 理解异常事实、呼叫 vs 维修边界 | 本页：准备 → 一笔异常如何记录 → 写实示例 → 边界 → 验证点 |
| 弄清无人到岗/来源错选如何排查 | 本页「关键判断」「建议验证点」；响应配置见问题响应 |
| 登记/关闭呼叫、查过程、停机项目操作 | [故障记录-维护与查询参考](故障记录-维护与查询参考.md) |
| 配谁响应、多久升级 | [问题响应](../02-问题响应/index.md) |
| 设备侧要开维修工单 | EAM [设备管理](../../08-EAM-设备管理/02-设备管理/index.md) |
...
!!! example "📝 示例数据占位"
    产线呼叫设备异常 → 过程到岗 → 结束呼叫 → 手工开 EAM 报修（对照两套时长）。

!!! example "写实示例：给定配置 → 期望行为"
    **给定：** 问题分类=设备，来源=安东异常；响应链已配班组长岗位与消息；设备 EQ-001；现场需维修但不期望自动开单。
    **期望：**

    1. 建立问题主记录后，过程记录出现通知与到岗痕迹；分类与响应链不一致时可能无人通知。
    2. 问题状态：呼叫中 → 结束；总体状态可另走审结。结束呼叫不等于 EAM 维修完成。
    3. 「维修单状态」若填写，仅为线索，**不**自动创建维修工单；需维修时手工开 EAM 报修并关联设备编码。
    4. 呼叫时长用于响应 SLA；维修工时在 EAM 统计，二者勿混算 OEE/MTTR。

### 建议验证点

- 建一条主记录并结束呼叫；过程可见通知与到岗。
- 分类故意配错：确认无人通知或通知错岗，再改回链配置。
- 来源分别选安东/巡检/质量的样例可区分统计。
- 需要维修时手工开 EAM 报修；确认未因呼叫自动生成维修工单（当前无强制自动）。
- 停机项目与 EAM「设备停机」单据区分选用。

## 与 EAM / MES / QMS 边界

「异常」这个词横跨安灯、维修、质量三个系统，混用最容易导致统计失真：
