...
订单建不了、价格算不对——往往不是流程问题，而是 SCP 侧的基础数据没配齐。基础数据为 SCP 协同单据提供供应商、物料、价格、包装、仓库/月台、单据开关、策略与供应商用户等配置底座；对象落在 SCP 自有表（如供应商、供应商物料、采购价格、仓库/库区/库位/月台、单据开关等），**与 DBC 主数据并存**，培训时勿默认「改 SCP 等于改 DBC」。读完本页，你能判断一个协同字段该改 SCP 还是该回 DBC 改。
...
「改 SCP 是否等于改 DBC」是这条边界上最常见的误解，下表逐条拆开：
