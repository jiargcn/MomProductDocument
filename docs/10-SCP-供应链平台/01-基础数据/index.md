...
|--------|--------|------|------|----------|------|
| supplier_code | 供应商编码 | VARCHAR(50) | 必填 | [采购订单](../02-采购订单/index.md)关联（唯一标识） | |
...
| material_code | 物料编码 | VARCHAR(50) | 必填 | [采购订单](../02-采购订单/index.md)引用 | |
| material_name | 物料名称 | VARCHAR(200) | 必填 | 显示 | |
| unit_price | 单价 | DECIMAL(12,4) | 必填 | [采购订单](../02-采购订单/index.md)计价 | 含税单价 |
...
2. **供应物料价格有效期**：超过有效期的价格记录自动失效，新建[采购订单](../02-采购订单/index.md)使用最新有效价格
3. **仓库建模层级完整性**：仓库→库区→库位组→库位为强制层级，不可跨层级挂靠
4. **[单据开关](../../04-DBC-主数据管理/05-策略设置/05-单据开关.md)影响范围**：关闭某[单据开关](../../04-DBC-主数据管理/05-策略设置/05-单据开关.md)后，所有相关业务入口不可见
