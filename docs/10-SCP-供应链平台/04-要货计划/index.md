...
| plan_qty | 计划数量 | DECIMAL(12,4) | 必填 | [采购订单](../02-采购订单/index.md)数量依据 | |
| plan_date | 计划日期 | DATE | 必填 | 采购执行时间 | |
| status | 状态 | ENUM | 字典项 | [采购订单](../02-采购订单/index.md)生成 | 待审核/已发布/已转单 |
...
1. **汇总规则**：同一物料、同一周期的多个已确认预测，按数量汇总生成一条[要货计划](index.md)
2. **计划排程**：[要货计划](index.md)按物料交期自动推算[采购订单](../02-采购订单/index.md)建议下单日期
3. **转单控制**：已发布的[要货计划](index.md)不可直接修改，需先撤销发布
4. **客户长期计划优先级**：客户长期计划与[要货预测](../03-要货预测/index.md)冲突时，以客户长期计划为准
