# 用户与权限

> 适用基线：测试环境目标 / `dev` 分支 / 2026-07-15。
> 阅读对象：系统管理员、实施顾问、安全协同人员及需要配置或排查授权问题的业务管理员。

## 这一组业务解决什么问题

用户与权限回答三件彼此相关、但不能混为一谈的事：

1. **功能授权（RBAC）**：谁能看到哪些菜单、页面和操作入口。
2. **数据权限**：在已有功能入口内，能看到或操作哪些数据范围。
3. **岗位与审批主体**：任务由谁承接、审批由谁完成，与功能授权如何配合。

本分组提供跨模块的授权底座说明。WMS、MES、QMS 等业务页仍要写清各自动作的状态条件、数据影响和审计要求，不能只写“有某某权限码”。

## 建议学习与操作顺序

| 顺序 | 建议先看什么 | 为什么 |
| --- | --- | --- |
| 1 | [RBAC 权限模型](01-RBAC权限模型.md) | 先建立用户—角色—菜单—权限标识主链。 |
| 2 | [RBAC权限模型-维护与查询参考](RBAC权限模型-维护与查询参考.md) | 学会日常维护与排错顺序。 |
| 3 | [数据权限与决策权限](02-数据权限与决策权限.md) | 区分“能进功能”和“能看数据/能做高风险动作”。 |
| 4 | [岗位、任务分配与审批主体](03-岗位、任务分配与审批主体.md) | 理解派工与审批主体，避免与角色功能授权混写。 |

## 关键对象关系

```mermaid
flowchart LR
    U["用户"] --> R["角色"]
    R --> M["菜单"]
    M --> P["权限标识"]
    R --> D["数据范围（另页）"]
    U --> O["组织/岗位（另页）"]
```

## 分页说明

| 页面 | 说明 | 本阶段状态 |
| --- | --- | --- |
| RBAC 权限模型 | 功能授权主链、超管、菜单/按钮/接口分层。 | 已业务化；含维护与查询参考。 |
| 数据权限与决策权限 | 数据范围与高风险决策动作边界。 | 第二阶段下一刀，当前保留边界说明。 |
| 岗位、任务分配与审批主体 | 岗位、任务承接与审批主体。 | 第二阶段后续切片。 |

## 常见问题（分组级）

| 情况 | 建议处理 |
| --- | --- |
| 用户说“没权限” | 先分清是看不见菜单、看不见按钮，还是执行时报错。 |
| 想靠加菜单解决数据隔离 | 回到数据权限页，不要只加功能入口。 |
| 业务动作在某状态不能做 | 回对应业务页查状态规则，不要只改角色。 |

## 当前限制与待确认事项

- 全站逐页 RBAC 实测矩阵尚未建立；
- 数据权限与岗位审批页仍待按同一文风回填；
- 前端显隐与后端强制鉴权不一致的接口清单需持续登记。
