...
> 阅读对象：系统管理员、实施顾问、安全与审计协同人员，以及需要理解“谁能看见什么、谁能点什么”的业务管理员。

## 业务目的与适用范围

RBAC（基于角色的访问控制）用于把“登录用户能使用系统的哪些功能”管理成可维护、可追溯的授权结果。它通过用户、角色、菜单和权限标识的组合，决定登录后能看到哪些页面，以及页面上哪些操作入口应显示。

本页说明 **用户 → 角色 → 菜单 → 权限标识** 的主链。数据可见范围（按部门等过滤）、岗位派工、审批主体和具体业务状态前置条件见同组其它页面，不在本页展开。业务模块中的收货、发料、盘点等动作规则仍以各业务页为准；本页只提供跨模块可复用的授权模型。
...
| --- | --- |
| 想理解用户如何获得菜单和操作入口 | 本页。 |
| 正在维护用户、角色、菜单或做角色授权 | [RBAC权限模型-维护与查询参考](RBAC权限模型-维护与查询参考.md)。 |
| 想区分“能进页面”和“能看哪些数据” | [数据权限与决策权限](02-数据权限与决策权限.md)。 |
| 想理解岗位、任务分配与审批人 | [岗位、任务分配与审批主体](03-岗位、任务分配与审批主体.md)。 |
...
【截图占位：角色管理页勾选菜单树，标出目录、页面和权限标识；使用脱敏测试数据。】
...
【示例数据占位：用户张三分配“仓库执行”角色，角色勾选采购收货任务页及执行相关权限标识；登录后可见收货任务菜单，按钮显示以权限标识为准。】
...
- 全站“页面—角色—按钮—接口”实测矩阵尚未建立，不能对每个业务动作给出最终可执行结论；
