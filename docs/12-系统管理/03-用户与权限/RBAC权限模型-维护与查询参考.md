...
> 阅读对象：测试、实施、运维（主）。
> 用途：配合[RBAC 权限模型](01-RBAC权限模型.md)使用。本页按用户、角色、菜单和授权操作给出维护与查询要点；读完应能按排查顺序定位「看不见菜单/按钮」，并完成角色授权验收。数据权限配置不在本页。

## 如何使用本页

| 你的目的 | 建议阅读 |
| --- | --- |
| 理解 RBAC 主链与三层边界 | 回 [RBAC 权限模型](01-RBAC权限模型.md) |
...
!!! example "写实示例：给定配置 → 期望可见范围"
    **给定：** 新建角色「仅查看收货」只勾选收货相关只读菜单；用户丁仅分配该角色；菜单启用。
    **期望：** 丁可见只读收货入口；执行类按钮因未下发对应权限标识而不显示。若仍看到执行按钮，核对菜单上是否误挂了执行标识，或是否误用超管账号验收。
