...
组织架构回答：系统里用哪些组织对象表达责任归属与人员挂载，以及它们如何与权限、主数据隔离配合。本页聚焦 **System 侧已证实的部门树与用户归属**，并划清与工厂、仓库、岗位的边界。
...
【截图占位：部门管理树形列表，标出上级部门、负责人、状态；使用脱敏数据。】
...
【示例数据占位：总部 → 仓储部 → 一号库班组；两名用户分属不同下级部门，在“本部门”范围下可见结果不同。】
