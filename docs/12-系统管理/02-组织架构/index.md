...
新建了「仓储部」却发现工厂建模里什么都没变——这是最常见的误会：部门树不等于工厂/仓库。组织架构回答：系统里用哪些组织对象表达责任归属与人员挂载，以及它们如何与权限、主数据隔离配合；本页聚焦 **System 侧部门树与用户归属**，并划清与工厂、仓库、岗位的边界。读完本页，你能判断一个组织类问题该改部门树，还是该去 DBC/WMS 改业务地点。

旧版规划中的“企业/组织—工厂—部门”三级，不能直接等同于当前 System 菜单模型：工厂、仓库等业务地点对象主要在 DBC/WMS 维护；System 组织入口以部门树和岗位为主。

## 如何使用本组文档

部门、岗位、工厂分属不同系统，先按目的挑对应页再动手：
